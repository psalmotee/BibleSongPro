...
function getEffectiveContentTab() {
  ...
  if (sidebarTab === "text") return "text";
...
function updateBgModeUi() {
  ...
  if (quick)
    quick.style.display = bgMode === "gradient" ? "none" : "inline-block";
  if (split)
    split.style.display = bgMode === "gradient" ? "inline-flex" : "none";
  updateBgModePicker();
}
...
function updateBgModePicker() {
  ...
  if (gradBtn)
    gradBtn.classList.toggle("active", isOn && bgMode === "gradient");
  if (offBtn) {
...
function handleBgTypeChange() {
  ...
  document.getElementById("bg-image-settings").style.display =
    t === "image" ? "block" : "none";
  document.getElementById("bg-video-settings").style.display =
    t === "video" ? "block" : "none";
  document.getElementById("bg-media-effects").style.display =
    t === "image" || t === "video" ? "block" : "none";
...
function updateBgTypePicker() {
  ...
  document
    .getElementById("bgtype-color")
    .classList.toggle("active", val === "color");
  document
    .getElementById("bgtype-image")
    .classList.toggle("active", val === "image");
  document
    .getElementById("bgtype-video")
    .classList.toggle("active", val === "video");
}

function handleBgImageSourceChange() {
  const src = document.getElementById("bg-image-source").value;
  document.getElementById("bg-url-row").style.display =
    src === "url" ? "block" : "none";
  document.getElementById("bg-upload-row").style.display =
    src === "upload" ? "block" : "none";
}

function handleBgVideoSourceChange() {
  const src = document.getElementById("bg-video-source").value;
  document.getElementById("bg-video-url-row").style.display =
    src === "url" ? "block" : "none";
  document.getElementById("bg-video-upload-row").style.display =
    src === "upload" ? "block" : "none";
}
...
function getScheduleLineHint(entry) {
  ...
  const clamp = (value) =>
    Math.max(1, Math.min(getMaxLinesForCurrentTab("bible"), value));
...
function getLtBgHeightPct(verseCount) {
  ...
  if (autoAdjustLtHeight && getEffectiveContentTab() === "bible") {
    const lineHeight = Number(
      document.getElementById("line-height-lt").value || 1.1,
    );
