...
function getItemSearchableText(item) {
  if (!item) return "";
  if (isBibleItem(item)) return normalizeSearchText(item.title || "");
...
function buildTextRecord(textDoc, { isNew = false } = {}) {
  const now = Date.now();
  const title = String(textDoc?.title || "").trim() || "Untitled Text";
  const text = String(textDoc?.content || textDoc?.text || "");
  const id = String(textDoc?.id || createId("text", title));
  return {
    id,
    type: "text",
    title,
    ...
    normalizedLines: normalizeSongLyricsLineBreaks(text).split("\n"),
    createdAt: isNew ? now : Number(textDoc?.createdAt || now) || now,
    updatedAt: now,
  };
}
...
function hydrateTextFromRecord(record) {
  return {
    ...
    searchableText:
      record.searchableText ||
      normalizeSearchText(`${record.title || ""}\n${record.text || ""}`),
    normalizedLines: Array.isArray(record.normalizedLines)
      ? record.normalizedLines
...
function scheduleTextPersist(textDoc) {
  if (!textDoc) return;
  clearTimeout(textSaveTimer);
  textSaveTimer = setTimeout(() => {
    const record = buildTextRecord(textDoc);
    textDoc.id = record.id;
    textDoc.searchableText = record.searchableText;
    textDoc.createdAt = record.createdAt;
    textDoc.updatedAt = record.updatedAt;
    idbPut(STORE_TEXTS, record).catch(() => {});
  }, 400);
}
...
function applyProjectionSettingsSnapshot(profile, opts = {}) {
  ...
  if (next.fullOffsetX != null && document.getElementById("full-offset-x"))
    document.getElementById("full-offset-x").value = next.fullOffsetX;
  if (next.fullOffsetY != null && document.getElementById("full-offset-y"))
    document.getElementById("full-offset-y").value = next.fullOffsetY;
