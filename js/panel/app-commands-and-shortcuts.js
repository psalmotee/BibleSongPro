function clearOutput(opts = {}) {
  ...
  if (isVmixMode()) {
    vmixAfterClear().catch((error) => {
      ...
      vmixLastError =
        error && error.message ? error.message : "vMix clear failed";
      updateVmixStatusUi();
    });
  }
}
...
function confirmReset() {
  showConfirm(
    t("settings_reset_plugin"),
    t("settings_reset_plugin_confirm"),
    () => {
      resetAppData();
    },
  );
...
function createNewItem() {
  ...
  const fetchedLyrics =
    lookupKey === newSongFetchedLookupKey ? newSongFetchedLyrics : "";
  ...
  saveToStorageDebounced();
  showToast(
    fetchedLyrics ? t("song_created_with_fetched_lyrics") : t("song_created"),
  );
}
...
function createNewTextItem() {
  ...
  const displayTitle = title || "Untitled Text";
  const newText = {
    id: createId("text", displayTitle),
    title: displayTitle,
    content,
    text: content,
    contentType: "text",
    searchableText: normalizeSearchText(`${displayTitle}\n${content}`),
    normalizedLines: normalizeSongLyricsLineBreaks(content).split("\n"),
    createdAt: Date.now(),
    updatedAt: Date.now(),
  };
  texts.push(newText);
  ...
  const searchInput = document.getElementById("song-search");
  if (searchInput) searchInput.value = "";
  setSearchValueForTab("text", "");
  selectItem(texts.length - 1);
  setEditorMode("text");
  const editor = document.getElementById("lyric-editor");
  if (editor) editor.focus();
  if (titleInput) titleInput.value = "";
  if (contentInput) contentInput.value = "";
  closeModal("newTextModal");
  saveState();
  saveToStorageDebounced();
  showToast(t("common_created"));
}
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (!isMod && !e.altKey && !e.shiftKey && keyLower === "a") {
      ...
      if (typeof setAnnotateMode === "function") {
        const nextAnnotateState = !(
          typeof annotateMode !== "undefined" && annotateMode
        );
        setAnnotateMode(nextAnnotateState);
        if (nextAnnotateState && typeof positionAnnotateToolbar === "function")
    ...
    if (!isMod && !e.altKey && !e.shiftKey && keyLower === "d") {
      ...
      if (panel) {
        ...
        if (willOpen && typeof positionDualVersionPanel === "function")
    ...
    if (e.key === "Tab" && !e.ctrlKey && !e.metaKey && !e.altKey) {
      ...
      const next =
        footerTabs[(idx + direction + footerTabs.length) % footerTabs.length];
