...
function syncAppStateFromUi() {
  ...
  const selectedSongId =
    !isBible && currentItem ? currentItem.id || null : null;
  ...
  appState = {
    ...
    live: {
      ...
      lastLiveState:
        lastLiveState ||
        (appState.live ? appState.live.lastLiveState : { kind: "clear" }),
    },
    ...
    audioMixer: {
      ...
      mediaAvSyncMs: Math.max(
        -200,
        Math.min(400, Math.round(Number(_pgmMediaLocalAvSyncMs) || 0)),
      ),
      ...
      ullUnderruns: Math.max(
        0,
        Math.round(Number(_pgmUltraLowLatencyUnderruns) || 0),
      ),
  ...
  if (sidebarTab === "bible") {
    if (activeBibleVersion && bibles[activeBibleVersion]) {
      ...
      if (idx !== -1) {
        ...
        let targetIdx = -1;
        if (verseStart) {
          targetIdx = pages.findIndex((p) =>
            matchesVerseStart(p.raw, verseStart),
          );
        }
        if (targetIdx === -1) {
          targetIdx = Math.max(
            0,
            Math.min(navBible.lineCursor || 0, pages.length - 1),
          );
        }
    ...
    if (idx !== -1) {
      ...
      lineCursor = Math.max(
        0,
        Math.min(navSong.lineCursor || 0, pages.length - 1),
      );
      updateButtonView({ preserveScroll: true, skipAutoScroll: true });
    }
  } else if (sidebarTab === "text") {
    const targetId = state.selectedSongId;
    const idx = targetId
      ? texts.findIndex((textItem) => textItem.id === targetId)
      : -1;
    if (idx !== -1) {
      selectItem(idx, { preserveLineCursor: true, skipButtonView: true });
    }
...
function bindNavMirrorSearch() {
  ...
  const mirrorHistoryBtn = document.getElementById(
    "nav-mirror-ref-history-btn",
  );
  ...
  if (!resultsBox.dataset.eventsBound) {
    ...
    resultsBox.addEventListener("mousedown", (e) => {
      const target = e.target.closest(
        ".nav-mirror-result-item, .nav-mirror-close-btn",
      );
    ...
    resultsBox.addEventListener("click", (e) => {
      ...
      const sourceItems = Array.isArray(resultsBox._sourceItems)
        ? resultsBox._sourceItems
        : [];
  ...
  const refreshNavMirrorResults = () => {
    ...
    const sourceItems =
      sourceList && Array.isArray(sourceList._navMirrorItems)
        ? sourceList._navMirrorItems
        : Array.from(document.querySelectorAll("#song-list .song-item")).map(
            (row) => ({
              row,
              title:
                row.querySelector(".search-title")?.textContent ||
    ...
    visibleItems.forEach((srcItem, i) => {
      ...
      row.className =
        "song-item nav-mirror-result-item" + (srcItem.active ? " active" : "");
  ...
  mirror.addEventListener("keydown", (e) => {
    if (e.key === "Enter") {
      const items = Array.from(
        resultsBox.querySelectorAll(".nav-mirror-result-item"),
      );
      if (
        resultsBox.style.display !== "none" &&
        mirrorResultIdx >= 0 &&
        items[mirrorResultIdx]
      ) {
        e.preventDefault();
  ...
  mirror.addEventListener("keydown", (e) => {
    ...
    items.forEach((el, i) =>
      el.classList.toggle("ac-active", i === mirrorResultIdx),
    );
  ...
  mirror.addEventListener("keydown", (e) => {
    ...
    if (isSpaceKey && sidebarTab === "bible") {
      ...
      const match =
        textBefore.match(/^([\p{L}1-3 ]+)\s+(\d+)$/u) ||
      ...
      if (match) {
        ...
        if (activeBibleVersion && bibles[activeBibleVersion]) {
          ...
          isValidBook = bibles[activeBibleVersion].some((c) => {
            const bookPart = normalizeSearchText(
              c.title.split(" ").slice(0, -1).join(" "),
            );
            const compactBookPart = bookPart.replace(/\s+/g, "");
            return (
              bookPart === bookCandidate ||
              (bookCandidate.length >= 3 &&
                bookPart.startsWith(bookCandidate)) ||
              compactBookPart === compactCandidate ||
              (compactCandidate.length >= 3 &&
                compactBookPart.startsWith(compactCandidate))
            );
          });
          if (!isValidBook) {
            isValidBook = Object.values(BIBLE_BOOKS).some((b) => {
              ...
              return (
                normalized === bookCandidate ||
                (bookCandidate.length >= 3 &&
                  normalized.startsWith(bookCandidate)) ||
                compactNormalized === compactCandidate ||
                (compactCandidate.length >= 3 &&
                  compactNormalized.startsWith(compactCandidate))
              );
            });
          }
        }
        if (isValidBook) {
          ...
          mirror.selectionStart = mirror.selectionEnd =
            normalizedRefPrefix.length + 1;
          main.value = mirror.value;
          handleSearch();
          refreshNavMirrorResults();
        }
  ...
  document.addEventListener("click", (e) => {
    if (
      !e.target.closest("#nav-mirror-search") &&
  ...
  window.addEventListener(
    ...
    (e) => {
      ...
      if (key === "a") {
        ...
        const ce =
          activeEl instanceof Element && activeEl.isContentEditable
            ? activeEl
...
function _defaultBibleChapterIndex(versionId) {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    const bookName = norm(
      item.book || String(item.title || "").replace(/\s+\d+\s*$/, ""),
    );
    if (!bookName) continue;
    if (
      genesisAliases.has(bookName) ||
      bookName.startsWith("genesis ") ||
      bookName.startsWith("gen ")
    ) {
      return i;
    }
  }
...
function _updateAutocomplete() {
  ...
  if (!box || !input) return;
  if (sidebarTab !== "bible" || !_acBooks.length) {
  ...
  if (
    !matches.length ||
    (matches.length === 1 && normalizeSearchText(matches[0]) === q)
  ) {
    box.style.display = "none";
    _acIdx = -1;
    return;
...
function applyUiSnapshot(ui) {
  ...
  ltModeUserPresets = Array.isArray(ui.ltModeUserPresets)
    ? ui.ltModeUserPresets
        ...
        .map((preset) => ({
          ...
          values:
            typeof normalizeLtPresetValues === "function"
  ...
  applyWorkspaceLayoutMode(ui.workspaceLayoutMode || "focused", {
    persist: false,
  });
  bibleRecentRefs = sanitizeBibleRefEntries(
    ui.bibleRecentRefs,
  ...
  renderMirrorBibleReferencePicker();
  if (ui.fontFamily)
    document.getElementById("font-family").value = ui.fontFamily;
  if (ui.fontWeight)
    document.getElementById("font-weight").value = ui.fontWeight;
  ...
  if (
    ...
    try {
      const sample =
        bibles[activeBibleVersion][0] && bibles[activeBibleVersion][0].verseText
          ? bibles[activeBibleVersion][0].verseText
          : bibles[activeBibleVersion][0] && bibles[activeBibleVersion][0].text
            ? bibles[activeBibleVersion][0].text
            : "";
  ...
  if (ui.fontSizeFull != null)
    document.getElementById("font-size-val").value = ui.fontSizeFull;
  ...
  if (ui.refFontSize != null)
    document.getElementById("ref-font-size-val").value = ui.refFontSize;
  if (ui.refBgColor) {
    ltRefBgColor = ui.refBgColor;
    document.getElementById("ref-bg-color").value = ui.refBgColor;
    document.getElementById("ref-bg-color-hex").value =
      ui.refBgColor.toUpperCase();
  }
  ...
  if (ui.autoResizeFull)
    document.getElementById("auto-resize-full").value = ui.autoResizeFull;
  if (ui.autoResizeLT && document.getElementById("auto-resize-lt"))
    document.getElementById("auto-resize-lt").value = ui.autoResizeLT;
  if (ui.refPositionFull && document.getElementById("ref-position-full"))
    document.getElementById("ref-position-full").value = ui.refPositionFull;
  ltStyle = ui.ltStyle || ltStyle;
  autoAdjustLtHeight =
    ui.autoAdjustLtHeight !== undefined
      ? ui.autoAdjustLtHeight
      : autoAdjustLtHeight;
  document.getElementById("auto-adjust-lt-height").checked = autoAdjustLtHeight;
  if (ui.showVersion != null)
    document.getElementById("show-version").checked = ui.showVersion;
  if (ui.shortenBibleVersions != null) {
    document.getElementById("shorten-bible-versions").checked =
      ui.shortenBibleVersions;
  } else {
    document.getElementById("shorten-bible-versions").checked = true;
  }
  if (ui.shortenBibleBooks != null)
    document.getElementById("shorten-bible-books").checked =
      ui.shortenBibleBooks;
  const songSolfaToggle = document.getElementById("show-song-solfa-notes");
  if (songSolfaToggle)
    songSolfaToggle.checked = ui.showSongSolfaNotes !== false;
  const songCategoryToggle = document.getElementById("show-song-category-name");
  if (songCategoryToggle)
    songCategoryToggle.checked = ui.showSongCategoryName !== false;
  const songSectionToggle = document.getElementById("display-song-sections");
  if (songSectionToggle)
    songSectionToggle.checked = ui.displaySongSections === true;
  if (ui.showVerseNos != null)
    document.getElementById("show-verse-nos").checked = ui.showVerseNos;
  ...
  if (ui.songTransitionType)
    document.getElementById("song-transition-type").value =
      ui.songTransitionType;
  if (ui.songTransitionDuration != null)
    document.getElementById("song-transition-duration").value =
      ui.songTransitionDuration;
  if (ui.animateBgTransitions != null)
    document.getElementById("animate-bg-transitions").checked =
      !!ui.animateBgTransitions;
  if (ui.ltStyles) {
  ...
  if (ltStyles.custom && !ltStyles.custom.nameKey)
    ltStyles.custom.nameKey = "style_name_custom_green";
  ...
  document.getElementById("font-upload-hint").innerText = customFonts.length
    ? t(
        customFonts.length > 1
          ? "settings_custom_fonts_loaded_plural"
          : "settings_custom_fonts_loaded",
      ).replace("{count}", String(customFonts.length))
    : t("settings_no_custom_font_loaded");
  renderFontFamilyOptions(
    ui.fontFamily || document.getElementById("font-family").value,
  );
  if (ui.bgType) document.getElementById("bg-type").value = ui.bgType;
  if (ui.bgImageSource)
    document.getElementById("bg-image-source").value = ui.bgImageSource;
  if (ui.bgImageUrl != null)
    document.getElementById("bg-image-url").value = ui.bgImageUrl;
  if (ui.bgUploadDataUrl != null) bgUploadDataUrl = ui.bgUploadDataUrl;
  if (ui.bgVideoSource)
    document.getElementById("bg-video-source").value = ui.bgVideoSource;
  if (ui.bgVideoUrl != null)
    document.getElementById("bg-video-url").value = ui.bgVideoUrl;
  if (ui.bgVideoUploadDataUrl != null)
    bgVideoUploadDataUrl = ui.bgVideoUploadDataUrl;
  if (ui.bgVideoLoop != null)
    document.getElementById("bg-video-loop").checked = !!ui.bgVideoLoop;
  if (ui.bgVideoSpeed != null)
    document.getElementById("bg-video-speed").value = ui.bgVideoSpeed;
  ...
  if (ui.bgOpacityFull != null) {
    ...
    if (!Number.isNaN(fullVal))
      bgOpacityFull = Math.max(0, Math.min(100, fullVal));
  }
  ...
  document.getElementById("song-bilingual-enabled").checked =
    !!ui.songBilingualEnabled;
  document.getElementById("song-display-mode").value = String(
    ui.songDisplayMode || DEFAULT_SONG_BILINGUAL_SETTINGS.displayMode,
  );
  document.getElementById("song-translation-mode").value = String(
    ui.songTranslationMode || DEFAULT_SONG_BILINGUAL_SETTINGS.translationMode,
  );
  document.getElementById("song-auto-translate-import").checked =
    ui.songAutoTranslateOnImport !== false;
  document.getElementById("song-auto-translate-open").checked =
    ui.songAutoTranslateOnOpen !== false;
  document.getElementById("song-translation-language").value = String(
    ui.songTargetLanguage || DEFAULT_SONG_BILINGUAL_SETTINGS.targetLanguage,
  );
  document.getElementById("song-translation-source-language").value = String(
    ui.songSourceLanguage || DEFAULT_SONG_BILINGUAL_SETTINGS.sourceLanguage,
  );
  document.getElementById("song-secondary-font-scale").value = String(
    ui.songSecondaryFontScale ||
      DEFAULT_SONG_BILINGUAL_SETTINGS.secondaryFontScale,
  );
  document.getElementById("song-cache-translations").checked =
    ui.songCacheTranslationsLocally !== false;
  document.getElementById("song-free-translation-api-url").value = String(
    ui.songFreeTranslationApiUrl ||
      DEFAULT_SONG_BILINGUAL_SETTINGS.freeTranslationApiUrl,
  );
  document.getElementById("song-translation-api-url").value = String(
    ui.songTranslationApiUrl ||
      DEFAULT_SONG_BILINGUAL_SETTINGS.translationApiUrl,
  );
  document.getElementById("song-translation-api-key").value = String(
    ui.songTranslationApiKey ||
      DEFAULT_SONG_BILINGUAL_SETTINGS.translationApiKey,
  );
  ...
  if (ui.textColor) {
    ...
    document.getElementById("text-color-hex").value =
      ui.textColor.toUpperCase();
  }
  if (ui.refColor) {
  ...
  if (ui.lineHeightFull)
    document.getElementById("line-height-full").value = ui.lineHeightFull;
  if (ui.lineHeightLT)
    document.getElementById("line-height-lt").value = ui.lineHeightLT;
  ...
  ltHAlignBibleVerse =
    ui.hAlignLTBibleVerse != null ? ui.hAlignLTBibleVerse : ltHAlignBibleVerse;
  ...
  document
    .getElementById("ratio-custom")
    .classList.toggle("active", activeRatio === "custom");
  updateLinePickerAvailability();
  updateCustomModeAvailability();
  updateTextEditorModeAvailability();
  if (typeof updateWorkspaceFontSizeControl === "function")
  ...
  if (activeRatio === "16-9" && !hasBgToggle)
    applyLtBgDefaultForTab(sidebarTab);
  ...
  if (ui.remoteShowPairCode != null) {
    ...
    if (pairCodeInput)
      pairCodeInput.value = String(ui.remoteShowPairCode || "").slice(0, 32);
  }
...
function setPresetPopoverOpen(nextOpen, opts = {}) {
  ...
  if (
    opts.persist !== false &&
    typeof saveFocusedWorkspaceControlsForTab === "function" &&
    typeof isFocusedWorkspaceMode === "function" &&
...
function togglePresetPopover() {
  if (typeof updateWorkspaceStateLabels === "function")
    updateWorkspaceStateLabels();
...
function positionPresetPopover() {
  ...
  const leftPos = Math.min(
    ...
    Math.max(8, viewportWidth - popWidth - 8),
  );
...
async function toggleOutputPopover() {
  ...
  const host = document.getElementById("output-screen-list-popover");
  if (host)
    host.innerHTML = `<div class="placeholder">${esc(t("output_detecting_displays"))}</div>`;
...
async function openStandaloneOutputOnScreen(screenId) {
  ...
  const chosen = (Array.isArray(outputScreenList) ? outputScreenList : []).find(
    (s) => s.id === screenId,
  );
  if (outputLiveActive) {
    ...
    showToast(
      chosen
        ? t("output_live_moved_to_display").replace(
            "{display}",
            chosen.label || "selected display",
          )
        : t("output_live_moved"),
    );
  } else {
    showToast(
      chosen
        ? t("output_selected_display").replace(
            "{display}",
            chosen.label || "output screen",
          )
...
function injectFontFace(font) {
  if (!font || !font.name || !font.dataUrl || loadedFontNames.has(font.name))
    return;
...
function saveState(label) {
  const state = {
    ...
    scenes:
      typeof _serializeScenes === "function" && typeof _scenes !== "undefined"
        ? _serializeScenes()
        : null,
    activeSceneId:
      typeof _activeSceneId !== "undefined" ? _activeSceneId : null,
    sceneIdCounter:
      typeof _sceneIdCounter !== "undefined" ? _sceneIdCounter : 0,
    sourceIdCounter:
      typeof _sourceIdCounter !== "undefined" ? _sourceIdCounter : 0,
  };
...
function _undoRestoreScenes(state) {
  ...
  try {
    _restoreScenes(
      state.scenes,
      state.activeSceneId,
      state.sceneIdCounter,
      state.sourceIdCounter,
    );
  } catch (e) {
    console.warn("[Undo] scene restore error:", e);
  }
}
...
function undoAction() {
  if (historyIndex > 0) {
    ...
    const msg = state.label
      ? t("common_undo_named").replace("{label}", state.label)
      : t("common_undo_successful");
    showToast(msg);
  } else {
...
function redoAction() {
  if (historyIndex < historyStates.length - 1) {
    ...
    saveToStorageDebounced();
    const msg = state.label
      ? t("common_redo_named").replace("{label}", state.label)
      : t("common_redo_successful");
    showToast(msg);
  } else {
...
function handleFontUpload(input) {
  ...
  files.forEach((file) => {
    ...
    reader.onload = () => {
      const baseName =
        (file.name || "Custom Font").replace(/\.[^.]+$/, "") || "Custom Font";
      ...
      document.getElementById("font-upload-hint").innerText =
...
function detectFontForCurrentBible() {
  ...
  const sampleItem = bibles[activeBibleVersion].find(
    (it) => it.text || it.verseText,
  );
  const sample = sampleItem
    ? sampleItem.verseText || sampleItem.text || ""
    : "";
  const suggested = detectScriptFromText(sample);
  if (suggested) {
    document.getElementById("font-family").value = suggested;
    showToast(
      t("bible_suggested_font").replace(
        "{font}",
        suggested.replace(/,.+$/, "").replace(/'|\\'/g, "").trim(),
      ),
    );
  } else {
...
function openStyleEditor(styleId) {
  ...
  const loadFromLiveBible = () => {
    ...
    const pg =
      pages[Math.max(0, Math.min(liveLineCursor || 0, pages.length - 1))];
  ...
  if (previewRefTextEl)
    previewRefTextEl.innerHTML = `${previewRef}${previewVer ? " (" + previewVer + ")" : ""}`;
  ...
  styleCanvasState.sample = {
    ref: previewRef,
    version: previewVer,
    verseHtml: previewBody,
  };

  const refBox = style.refBox ||
    ltStyles["custom"]?.refBox || {
      bgColor: "#00411C",
      borderColor: "#FFFFFF",
      borderRadius: 10,
      borderWidth: 2,
  ...
  const refFillType =
    refBox.fillType ||
    (refBox.gradientStart || refBox.gradientEnd ? "gradient" : "color");
  ...
  setVal(
    "se-ref-grad-start",
    refBox.gradientStart || refBox.bgColor || "#00411C",
  );
  setVal(
    "se-ref-grad-start-hex",
    refBox.gradientStart || refBox.bgColor || "#00411C",
  );
  setVal(
    "se-ref-grad-end",
    refBox.gradientEnd || refBox.gradientStart || refBox.bgColor || "#00411C",
  );
  setVal(
    "se-ref-grad-end-hex",
    refBox.gradientEnd || refBox.gradientStart || refBox.bgColor || "#00411C",
  ...
  setVal(
    "se-ref-pad-x",
    refBox.padX ?? refBox.paddingLeft ?? refBox.padding ?? 24,
  );
  setVal(
    "se-ref-pad-y",
    refBox.padY ?? refBox.paddingBottom ?? refBox.padding ?? 10,
  );
  ...
  const refOpacityVal = Math.round(
    refOpacityRaw > 1 ? refOpacityRaw : refOpacityRaw * 100,
  );
  setVal("se-ref-opacity", refOpacityVal);
  setTxt("val-ref-opacity", refOpacityVal + "%");

  const mainBar = style.mainBar ||
    ltStyles["custom"]?.mainBar || {
      gradientStart: "#AD0000",
      gradientEnd: "#000000",
      minHeight: 240,
      x: 0,
  ...
  if (seBarBgColor)
    seBarBgColor.value = mainBar.bgColor || mainBar.gradientStart || "#85142B";
  if (seBarBgHex)
    seBarBgHex.value = mainBar.bgColor || mainBar.gradientStart || "#85142B";
  ...
  const barPadding =
    mainBar.padding ?? mainBar.paddingLeft ?? mainBar.padX ?? 10;
  ...
  const barOpacityVal = Math.round(
    barOpacityRaw > 1 ? barOpacityRaw : barOpacityRaw * 100,
  );
  setVal("se-bar-opacity", barOpacityVal);
  setTxt("val-bar-opacity", barOpacityVal + "%");

  const textMain = style.textPropsMain ||
    style.textProps || {
      lineSpacing: 1.1,
      textTransform: "none",
      textAlign: "left",
  ...
  const textRef = style.textPropsRef ||
    style.textProps || {
      lineSpacing: 1.2,
      textTransform: "none",
      textAlign: "center",
  ...
  const seMainTextFontFamily = document.getElementById(
    "se-main-text-font-family",
  );
  if (seMainTextFontFamily)
    seMainTextFontFamily.value = textMain.fontFamily || "";
  ...
  const seMainTextFontWeight = document.getElementById(
    "se-main-text-font-weight",
  );
  if (seMainTextFontWeight)
    seMainTextFontWeight.value = textMain.fontWeight ?? 700;
  ...
  if (seMainTextColorHex)
    seMainTextColorHex.value = textMain.color || "#FFFFFF";
  ...
  const seMainTextLetterSpacing = document.getElementById(
    "se-main-text-letter-spacing",
  );
  if (seMainTextLetterSpacing)
    seMainTextLetterSpacing.value = textMain.letterSpacing ?? 0;
  const seMainTextWordSpacing = document.getElementById(
    "se-main-text-word-spacing",
  );
  if (seMainTextWordSpacing)
    seMainTextWordSpacing.value = textMain.wordSpacing ?? 0;
  const seMainTextWritingMode = document.getElementById(
    "se-main-text-writing-mode",
  );
  if (seMainTextWritingMode)
    seMainTextWritingMode.value = textMain.writingMode || "horizontal-tb";
  const seMainTextOrientation = document.getElementById(
    "se-main-text-orientation",
  );
  if (seMainTextOrientation)
    seMainTextOrientation.value = textMain.textOrientation || "mixed";
  ...
  // Main Text shadow
  const mainShadow = textMain.shadow || {
    color: "#000000",
    opacity: 0,
    blur: 8,
    offsetX: 2,
  ...
  const seMainTextShadowColor = document.getElementById(
    "se-main-text-shadow-color",
  );
  const seMainTextShadowColorHex = document.getElementById(
    "se-main-text-shadow-color-hex",
  );
  const seMainTextShadowOpacity = document.getElementById(
    "se-main-text-shadow-opacity",
  );
  const seMainTextShadowBlur = document.getElementById(
    "se-main-text-shadow-blur",
  );
  const seMainTextShadowOffset = document.getElementById(
    "se-main-text-shadow-offset",
  );
  if (seMainTextShadowColor)
    seMainTextShadowColor.value = mainShadow.color || "#000000";
  if (seMainTextShadowColorHex)
    seMainTextShadowColorHex.value = mainShadow.color || "#000000";
  if (seMainTextShadowOpacity)
    seMainTextShadowOpacity.value = mainShadowOpacityVal;
  if (seMainTextShadowBlur) seMainTextShadowBlur.value = mainShadow.blur ?? 8;
  if (seMainTextShadowOffset)
    seMainTextShadowOffset.value = mainShadow.offsetX ?? 2;
  ...
  if (seRefTextHAlign)
    seRefTextHAlign.value = textRef.hAlign || textRef.textAlign || "center";
  ...
  const seRefTextFontFamily = document.getElementById(
    "se-ref-text-font-family",
  );
  ...
  const seRefTextFontWeight = document.getElementById(
    "se-ref-text-font-weight",
  );
  if (seRefTextFontWeight)
    seRefTextFontWeight.value = textRef.fontWeight ?? 700;
  ...
  const seRefTextLetterSpacing = document.getElementById(
    "se-ref-text-letter-spacing",
  );
  if (seRefTextLetterSpacing)
    seRefTextLetterSpacing.value = textRef.letterSpacing ?? 0;
  const seRefTextWordSpacing = document.getElementById(
    "se-ref-text-word-spacing",
  );
  if (seRefTextWordSpacing)
    seRefTextWordSpacing.value = textRef.wordSpacing ?? 0;
  ...
  const refTextShadow = textRef.shadow || {
    color: "#000000",
    opacity: 0,
    blur: 8,
    offsetX: 2,
  };
  const refTextShadowOpacityVal = Math.round(
    (refTextShadow.opacity ?? 0) * 100,
  );
  const seRefTextShadowColor = document.getElementById(
    "se-ref-text-shadow-color",
  );
  const seRefTextShadowColorHex = document.getElementById(
    "se-ref-text-shadow-color-hex",
  );
  const seRefTextShadowOpacity = document.getElementById(
    "se-ref-text-shadow-opacity",
  );
  const seRefTextShadowBlur = document.getElementById(
    "se-ref-text-shadow-blur",
  );
  const seRefTextShadowOffset = document.getElementById(
    "se-ref-text-shadow-offset",
  );
  if (seRefTextShadowColor)
    seRefTextShadowColor.value = refTextShadow.color || "#000000";
  if (seRefTextShadowColorHex)
    seRefTextShadowColorHex.value = refTextShadow.color || "#000000";
  if (seRefTextShadowOpacity)
    seRefTextShadowOpacity.value = refTextShadowOpacityVal;
  if (seRefTextShadowBlur) seRefTextShadowBlur.value = refTextShadow.blur ?? 8;
  if (seRefTextShadowOffset)
    seRefTextShadowOffset.value = refTextShadow.offsetX ?? 2;
  ...
  const seBarShadowColorHex = document.getElementById(
    "se-bar-shadow-color-hex",
  );
  ...
  if (seBarShadowColorHex)
    seBarShadowColorHex.value = shadow.color || "#000000";
  ...
  const versionBar = style.versionBar ||
    ltStyles["custom"]?.versionBar || {
      bgColor: "#1a1a2e",
  ...
  const seVersionBarFillType = document.getElementById(
    "se-version-bar-fill-type",
  );
  ...
  const seVersionBarBgColor = document.getElementById(
    "se-version-bar-bg-color",
  );
  const seVersionBarBgHex = document.getElementById("se-version-bar-bg-hex");
  if (seVersionBarBgColor)
    seVersionBarBgColor.value = versionBar.bgColor || "#1a1a2e";
  if (seVersionBarBgHex)
    seVersionBarBgHex.value = versionBar.bgColor || "#1a1a2e";
  const seVersionBarGradStart = document.getElementById(
    "se-version-bar-grad-start",
  );
  const seVersionBarGradStartHex = document.getElementById(
    "se-version-bar-grad-start-hex",
  );
  if (seVersionBarGradStart)
    seVersionBarGradStart.value = versionBar.gradientStart || "#1a1a2e";
  if (seVersionBarGradStartHex)
    seVersionBarGradStartHex.value = versionBar.gradientStart || "#1a1a2e";
  const seVersionBarGradEnd = document.getElementById(
    "se-version-bar-grad-end",
  );
  const seVersionBarGradEndHex = document.getElementById(
    "se-version-bar-grad-end-hex",
  );
  if (seVersionBarGradEnd)
    seVersionBarGradEnd.value = versionBar.gradientEnd || "#16213e";
  if (seVersionBarGradEndHex)
    seVersionBarGradEndHex.value = versionBar.gradientEnd || "#16213e";
  const seVersionBarBorderColor = document.getElementById(
    "se-version-bar-border-color",
  );
  const seVersionBarBorderHex = document.getElementById(
    "se-version-bar-border-hex",
  );
  if (seVersionBarBorderColor)
    seVersionBarBorderColor.value = versionBar.borderColor || "#FFFFFF";
  if (seVersionBarBorderHex)
    seVersionBarBorderHex.value = versionBar.borderColor || "#FFFFFF";
  const seVersionBarRadius = document.getElementById("se-version-bar-radius");
  if (seVersionBarRadius)
    seVersionBarRadius.value = versionBar.borderRadius ?? 8;
  const seVersionBarBorderWidth = document.getElementById(
    "se-version-bar-border-width",
  );
  if (seVersionBarBorderWidth)
    seVersionBarBorderWidth.value = versionBar.borderWidth ?? 1;
  ...
  if (seVersionBarY)
    seVersionBarY.value = versionBar.y ?? versionBar.yOffset ?? -50;
  const versionBarOpacityRaw = versionBar.opacity ?? 0.95;
  const versionBarOpacityVal = Math.round(
    versionBarOpacityRaw > 1
      ? versionBarOpacityRaw
      : versionBarOpacityRaw * 100,
  );
  ...
  if (seVersionBarOpacity) {
    ...
    const valVersionBarOpacity = document.getElementById(
      "val-version-bar-opacity",
    );
    if (valVersionBarOpacity)
      valVersionBarOpacity.innerText = versionBarOpacityVal + "%";
  }
  ...
  if (seVersionBarHAlign)
    seVersionBarHAlign.value = versionBar.hAlign || "center";
  const seVersionBarVAlign = document.getElementById("se-version-bar-v-align");
  if (seVersionBarVAlign)
    seVersionBarVAlign.value = versionBar.vAlign || "middle";
  const seVersionBarRotation = document.getElementById(
    "se-version-bar-rotation",
  );
  if (seVersionBarRotation)
    seVersionBarRotation.value = versionBar.rotation ?? 0;
  // Version Bar shadow
  const seVersionBarShadowColor = document.getElementById(
    "se-version-bar-shadow-color",
  );
  const seVersionBarShadowOpacity = document.getElementById(
    "se-version-bar-shadow-opacity",
  );
  const seVersionBarShadowBlur = document.getElementById(
    "se-version-bar-shadow-blur",
  );
  const seVersionBarShadowOffset = document.getElementById(
    "se-version-bar-shadow-offset",
  );
  if (seVersionBarShadowColor)
    seVersionBarShadowColor.value = shadow.color || "#000000";
  if (seVersionBarShadowOpacity)
    seVersionBarShadowOpacity.value = shadowOpacityVal;
  if (seVersionBarShadowBlur) seVersionBarShadowBlur.value = shadow.blur ?? 8;
  if (seVersionBarShadowOffset)
    seVersionBarShadowOffset.value = shadow.offsetX ?? 2;
  // Version Bar auto-resize
  const seVersionBarAutoResize = document.getElementById(
    "se-version-bar-auto-resize",
  );
  ...
  const seVersionTextFontFamily = document.getElementById(
    "se-version-text-font-family",
  );
  if (seVersionTextFontFamily)
    seVersionTextFontFamily.value = textVersion.fontFamily || "";
  const seVersionTextFontSize = document.getElementById(
    "se-version-text-font-size",
  );
  if (seVersionTextFontSize)
    seVersionTextFontSize.value = textVersion.fontSize ?? 24;
  const seVersionTextFontWeight = document.getElementById(
    "se-version-text-font-weight",
  );
  if (seVersionTextFontWeight)
    seVersionTextFontWeight.value = textVersion.fontWeight ?? 600;
  const seVersionTextColor = document.getElementById("se-version-text-color");
  const seVersionTextColorHex = document.getElementById(
    "se-version-text-color-hex",
  );
  if (seVersionTextColor)
    seVersionTextColor.value = textVersion.color || "#FFFFFF";
  if (seVersionTextColorHex)
    seVersionTextColorHex.value = textVersion.color || "#FFFFFF";
  const lsVersion = textVersion.lineSpacing ?? 1.2;
  const seVersionLineSpacing = document.getElementById(
    "se-version-line-spacing",
  );
  if (seVersionLineSpacing) seVersionLineSpacing.value = lsVersion;
  const valVersionLineSpacing = document.getElementById(
    "val-version-line-spacing",
  );
  if (valVersionLineSpacing) valVersionLineSpacing.innerText = lsVersion;
  const seVersionTextTransform = document.getElementById(
    "se-version-text-transform",
  );
  if (seVersionTextTransform)
    seVersionTextTransform.value = textVersion.textTransform || "none";
  const seVersionTextHAlign = document.getElementById(
    "se-version-text-h-align",
  );
  if (seVersionTextHAlign)
    seVersionTextHAlign.value = textVersion.hAlign || "center";
  const seVersionTextVAlign = document.getElementById(
    "se-version-text-v-align",
  );
  if (seVersionTextVAlign)
    seVersionTextVAlign.value = textVersion.vAlign || "middle";
  ...
  const seVersionTextRotation = document.getElementById(
    "se-version-text-rotation",
  );
  if (seVersionTextRotation)
    seVersionTextRotation.value = textVersion.rotation ?? 0;
  const seVersionTextOpacity = document.getElementById(
    "se-version-text-opacity",
  );
  if (seVersionTextOpacity)
    seVersionTextOpacity.value = textVersion.opacity ?? 100;
  ...
  if (seVersionSingleLine)
    seVersionSingleLine.value = textVersion.singleLine ? "true" : "false";
  const seVersionLetterSpacing = document.getElementById(
    "se-version-text-letter-spacing",
  );
  if (seVersionLetterSpacing)
    seVersionLetterSpacing.value = textVersion.letterSpacing ?? 0;
  const seVersionWordSpacing = document.getElementById(
    "se-version-text-word-spacing",
  );
  if (seVersionWordSpacing)
    seVersionWordSpacing.value = textVersion.wordSpacing ?? 0;
  const seVersionWritingMode = document.getElementById(
    "se-version-text-writing-mode",
  );
  if (seVersionWritingMode)
    seVersionWritingMode.value = textVersion.writingMode || "horizontal-tb";
  const seVersionOrientation = document.getElementById(
    "se-version-text-orientation",
  );
  if (seVersionOrientation)
    seVersionOrientation.value = textVersion.textOrientation || "mixed";
  const seVersionTextAutoResize = document.getElementById(
    "se-version-text-auto-resize",
  );
  if (seVersionTextAutoResize)
    seVersionTextAutoResize.value = textVersion.autoResize || "grow";
  // Version Text shadow
  const versionTextShadow = textVersion.shadow ||
    style.shadow || { color: "#000000", opacity: 0, blur: 8, offsetX: 2 };
  const versionTextShadowOpacityVal = Math.round(
    (versionTextShadow.opacity ?? 0) * 100,
  );
  const seVersionTextShadowColor = document.getElementById(
    "se-version-text-shadow-color",
  );
  const seVersionTextShadowColorHex = document.getElementById(
    "se-version-text-shadow-color-hex",
  );
  const seVersionTextShadowOpacity = document.getElementById(
    "se-version-text-shadow-opacity",
  );
  const seVersionTextShadowBlur = document.getElementById(
    "se-version-text-shadow-blur",
  );
  const seVersionTextShadowOffset = document.getElementById(
    "se-version-text-shadow-offset",
  );
  if (seVersionTextShadowColor)
    seVersionTextShadowColor.value = versionTextShadow.color || "#000000";
  if (seVersionTextShadowColorHex)
    seVersionTextShadowColorHex.value = versionTextShadow.color || "#000000";
  if (seVersionTextShadowOpacity)
    seVersionTextShadowOpacity.value = versionTextShadowOpacityVal;
  if (seVersionTextShadowBlur)
    seVersionTextShadowBlur.value = versionTextShadow.blur ?? 8;
  if (seVersionTextShadowOffset)
    seVersionTextShadowOffset.value = versionTextShadow.offsetX ?? 2;
...
function syncStyleColor(type) {
  const pairs = {
    ...
    "version-bar-grad-start": [
      "se-version-bar-grad-start",
      "se-version-bar-grad-start-hex",
    ],
    "version-bar-grad-end": [
      "se-version-bar-grad-end",
      "se-version-bar-grad-end-hex",
    ],
    "version-bar-border": [
    ...
    "version-bar-shadow": [
      "se-version-bar-shadow-color",
      "se-version-bar-shadow-color-hex",
    ],
...
function toggleFillFields(prefix, fillType) {
  ...
  const gradStartField = document.getElementById(
    prefix + "-gradientStart-field",
  );
...
function styleCanvasReadControls() {
  ...
  const scaleValue = Number(
    scaleInput?.value || styleCanvasState.preview.scale || 0.45,
  );
...
function updateStyleCanvasStage() {
  ...
  if (dom.wrap && dom.frame) {
    const needsZoom =
      dom.frame.offsetWidth > dom.wrap.clientWidth ||
      dom.frame.offsetHeight > dom.wrap.clientHeight;
...
function fitStyleCanvasToWrap() {
  ...
  const scale = Math.min(
    width / styleCanvasBaseSize.width,
    height / styleCanvasBaseSize.height,
  );
...
function initStyleEditorCanvas() {
  ...
  const baseXInput = document.getElementById("base-x");
  if (baseXInput)
    baseXInput.addEventListener("input", () => updateStylePreview());
  const baseYInput = document.getElementById("base-y");
  if (baseYInput)
    baseYInput.addEventListener("input", () => updateStylePreview());
  const gridToggle = document.getElementById("grid-toggle");
  if (gridToggle)
    gridToggle.addEventListener("change", () => updateStylePreview());
  const safeToggle = document.getElementById("safe-toggle");
  if (safeToggle)
    safeToggle.addEventListener("change", () => updateStylePreview());
  const gridSizeInput = document.getElementById("grid-size");
  if (gridSizeInput)
    gridSizeInput.addEventListener("input", () => updateStylePreview());
...
function applyCategoryStateToLayer(layer, styleData) {
  ...
  if (layer.category === "ref_box") {
    ...
    layer.gradientEnd =
      box.gradientEnd || layer.gradientStart || box.bgColor || "#00411C";
  ...
  if (layer.category === "main_text") {
    ...
    layer.fontWeight =
      text.fontWeight != null ? text.fontWeight : layer.fontWeight;
    layer.writingMode =
      text.writingMode || layer.writingMode || "horizontal-tb";
    layer.textOrientation =
      text.textOrientation || layer.textOrientation || "mixed";
  ...
  if (layer.category === "ref_text") {
    ...
    layer.vAlign = text.vAlign || layer.vAlign || "middle";
    if (!layer.text) {
      const versionText = styleCanvasState.sample.version
        ? ` (${styleCanvasState.sample.version})`
        : "";
  ...
  if (layer.category === "version_text") {
    ...
    layer.fontFamily = text.fontFamily || layer.fontFamily;
    layer.fontSize =
      text.fontSize != null ? text.fontSize : layer.fontSize || 24;
    layer.fontWeight =
      text.fontWeight != null ? text.fontWeight : layer.fontWeight || 600;
    layer.writingMode =
      text.writingMode || layer.writingMode || "horizontal-tb";
    layer.textOrientation =
      text.textOrientation || layer.textOrientation || "mixed";
...
function renderStyleCanvasLayers(styleData) {
  ...
  layers.forEach((layer) => {
    ...
    if (
      ...
      const layerFillType =
        layer.fillType ||
        (layer.gradientStart || layer.gradientEnd ? "gradient" : "color");
    ...
    if (layer.type === "text") {
      ...
      if (layer.letterSpacing != null)
        el.style.letterSpacing = `${layer.letterSpacing}px`;
      if (layer.wordSpacing != null)
        el.style.wordSpacing = `${layer.wordSpacing}px`;
...
function renderStyleCanvasRefBox(styleData) {
  ...
  const refFillType =
    box.fillType ||
    (box.gradientStart || box.gradientEnd ? "gradient" : "color");
  ...
  const refFontSize =
    textRef.fontSize != null ? textRef.fontSize : box.fontSize || 36;
  const refFontWeight =
    textRef.fontWeight != null ? textRef.fontWeight : box.fontWeight || 700;
    ...
    hAlign === "left"
      ...
      : hAlign === "right"
        ...
        : hAlign === "justify"
          ? "stretch"
          : "center";
  dom.refBox.style.justifyContent =
    vAlign === "top"
      ? "flex-start"
      : vAlign === "bottom"
        ? "flex-end"
  ...
  const refSingleLine = !(
    textRef.singleLine === false || textRef.singleLine === "false"
  );
...
function renderStyleCanvasMainBar(styleData) {
  ...
  const barFillType =
    bar.fillType ||
    (bar.gradientStart || bar.gradientEnd ? "gradient" : "color");
  ...
  const mainFontWeight =
    textMain.fontWeight != null ? textMain.fontWeight : 700;
  ...
  const mainSingleLine =
    textMain.singleLine === true || textMain.singleLine === "true";
  ...
  if (textMain.opacity != null) {
    dom.mainText.style.opacity = styleCanvasNormalizeOpacity(
      textMain.opacity,
      1,
    );
  } else {
  ...
  dom.mainBar.style.alignItems =
    barVAlign === "top"
      ? "flex-start"
      : barVAlign === "bottom"
        ? "flex-end"
        : "center";
  dom.mainBar.style.justifyContent =
    barHAlign === "center"
      ? "center"
      : barHAlign === "right"
        ? "flex-end"
...
function updateStylePreview() {
  initStyleEditorCanvas();
  const dom = getStyleCanvasDom();
  if (!dom || !dom.refBox || !dom.mainBar || !dom.refText || !dom.mainText)
    return;
  ...
  const baseStyle =
    editingStyleId && ltStyles[editingStyleId]
      ? ltStyles[editingStyleId]
      : ltStyles["custom"] || {};
  ...
  const styleData = {
    ...
    versionBar: {
      ...(baseStyle.versionBar || {}),
      ...(formData.versionBar || {}),
    },
    textPropsMain: {
      ...(baseStyle.textPropsMain || {}),
      ...(formData.textPropsMain || {}),
    },
    textPropsRef: {
      ...(baseStyle.textPropsRef || {}),
      ...(formData.textPropsRef || {}),
    },
    textPropsVersion: {
      ...(baseStyle.textPropsVersion || {}),
      ...(formData.textPropsVersion || {}),
    },
...
function collectStyleFormData() {
  ...
  return {
    ...
    mainBar: {
      ...
      bgColor:
        getVal("se-bar-bg-color", null) ||
        getVal("se-bar-grad-start", "#AD0000"),
    ...
    textPropsRef: {
      ...
      textAlign:
        getVal("se-ref-text-h-align", null) ||
        getVal("se-ref-h-align", "center"),
...
function mergeStyleData(baseStyle, changes) {
  ...
  return {
    ...
    textPropsVersion: {
      ...(base.textPropsVersion || {}),
      ...(next.textPropsVersion || {}),
    },
    shadow: { ...(base.shadow || {}), ...(next.shadow || {}) },
    meta: { ...(base.meta || {}), ...(next.meta || {}) },
    layers: Array.isArray(next.layers)
      ? next.layers
...
function scheduleStyleAutosave() {
  ...
  styleAutosaveTimer = setTimeout(() => {
    ...
    ltStyles[editingStyleId] = mergeStyleData(
      ltStyles[editingStyleId],
      tempStyleChanges,
    );
    saveToStorageDebounced();
...
function saveStyleAsTemplate() {
  const baseName =
    (ltStyles[editingStyleId || ltStyle]?.name || "Custom Style") + " Copy";
  showConfirm(
    t("style_save_new_template"),
    t("style_give_name_prompt"),
    (name) => {
      const finalName =
...
function promptSavePreset() {
  showConfirm(
    ...
    (name) => {
      ...
      if (exists) {
        showConfirm(
          t("preset_replace_preset"),
          t("preset_exists_replace").replace("{name}", finalName),
          () => {
...
function savePreset(name, replaceExisting) {
  ...
  const payload = {
    id:
      replaceExisting && presets.find((p) => p.name === name)?.id
        ? presets.find((p) => p.name === name).id
        : `preset_${Date.now()}`,
    name,
    ui: snapshot,
  };
...
function deleteStyle(styleId) {
  ...
  showConfirm(
    "Delete Style",
    `Delete style "${ltStyles[styleId].name}"?`,
    () => {
  ...
  showConfirm(
    ...
    (name) => {
      ...
      const pendingChanges =
        tempStyleChanges && editingStyleId === ltStyle
          ? tempStyleChanges
          : null;
      const mergedStyle = pendingChanges
        ? mergeStyleData(currentStyleData, pendingChanges)
        : currentStyleData;
    ...
    try {
      ...
      if (styleData.template && !isLowerThirdDesigner) {
        ...
        ltStyles[styleId] = {
          ...
          type: "custom",
          deletable: true,
          refBox: styleData.refBox
            ? JSON.parse(JSON.stringify(styleData.refBox))
            : {},
          mainBar: styleData.mainBar
            ? JSON.parse(JSON.stringify(styleData.mainBar))
            : {},
          versionBar: styleData.versionBar
            ? JSON.parse(JSON.stringify(styleData.versionBar))
            : {},
          textPropsMain: styleData.textPropsMain
            ? JSON.parse(JSON.stringify(styleData.textPropsMain))
            : styleData.textProps
              ? JSON.parse(JSON.stringify(styleData.textProps))
              : {},
          textPropsRef: styleData.textPropsRef
            ? JSON.parse(JSON.stringify(styleData.textPropsRef))
            : styleData.textProps
              ? JSON.parse(JSON.stringify(styleData.textProps))
              : {},
          textPropsVersion: styleData.textPropsVersion
            ? JSON.parse(JSON.stringify(styleData.textPropsVersion))
            : {},
          shadow: styleData.shadow
            ? JSON.parse(JSON.stringify(styleData.shadow))
            : {},
          autoResize: styleData.autoResize || "none",
          layers: importedLayers,
          meta: styleData.meta
            ? JSON.parse(JSON.stringify(styleData.meta))
            : {},
        };

        console.log(
          "Imported Lower Third Designer style:",
          styleData.name,
          ltStyles[styleId],
        );
      }
      ...
      Promise.all([flushAppState(), persistLtStyles()]).finally(() => {
        showToast(
          `Style "${styleData.name || styleData.template?.name || "Imported"}" imported successfully`,
        );
      });
    } catch (e) {
...
function recoverLiveBiblePointerFromCurrentItem() {
  ...
  const list =
    version && bibles && Array.isArray(bibles[version])
      ? bibles[version]
      : null;
...
function setupPresetPopoverDismiss() {
  ...
  document.addEventListener("click", (e) => {
    ...
    if (
      ...
      outputBtn &&
      outputPopoverOpen &&
      !(outputPop.contains(e.target) || outputBtn.contains(e.target))
    ) {
...
function bindEventsOnce() {
  ...
  if (
    ...
    !window.__footerPrimaryTabsObserverBound
  ) {
...
function setupSidebarResizePersistence() {
  ...
  function getNarrowSidebarDefaultHeight() {
    ...
    const available = Math.max(
      100,
      window.innerHeight - topbarHeight - footerHeight,
    );
    return available;
  }
  ...
  function saveUnifiedLeftPanelWidth() {
    try {
      localStorage.setItem(
        unifiedLeftWidthKey,
        Math.round(getCurrentLeftPanelWidth()) + "px",
      );
    } catch (e) {}
  }
  ...
  function restoreUnifiedLeftPanelWidth() {
    ...
    let saved = "";
    try {
      saved = localStorage.getItem(unifiedLeftWidthKey) || "";
    } catch (e) {}
    ...
    try {
      const mainSaved = parseFloat(
        localStorage.getItem("mainPanelWidth") || "",
      );
      const altSaved = parseFloat(
        localStorage.getItem("leftWorkspaceWidth") || "",
      );
  ...
  function heightKey(tab) {
    return "sidebarTopHeight_" + (tab || sidebarTab || "bible");
  }
  function restoreSidebarHeight() {
    ...
    try {
      ...
      if (Number.isFinite(savedPx) && savedPx > 320)
        sidebar.style.height = saved;
  ...
  window._restoreSidebarTabHeight = function () {
  ...
  if (vHandle) {
    ...
    vHandle.addEventListener("pointermove", (e) => {
      if (!vDrag) return;
      const newH = Math.min(
        getNarrowSidebarDefaultHeight(),
        Math.max(100, vStartH + e.clientY - vStartY),
      );
    ...
    vHandle.addEventListener("pointerup", (e) => {
      ...
      try {
        localStorage.setItem(heightKey(), sidebar.style.height);
      } catch (e) {}
    });
  ...
  if (programHandle && main) {
    ...
    programHandle.addEventListener("pointerdown", (e) => {
      ...
      pDrag = true;
      pStartX = e.clientX;
      pStartW = getCurrentLeftPanelWidth();
  ...
  if (workspaceHandle && workspaceTop && studioDock) {
    ...
    const onWPointerDown = (e) => {
      ...
      wDrag = true;
      wStartY = e.clientY;
      wStartH = studioDock.offsetHeight;
    ...
    const onWPointerMove = (e) => {
      ...
      const newH = Math.min(
        maxDock,
        Math.max(minDock, wStartH - (e.clientY - wStartY)),
      );
    ...
    const onWPointerUp = (e) => {
      ...
      document.body.style.userSelect = "";
      try {
        localStorage.setItem("studioDockHeight", studioDock.style.height);
      } catch (e) {}
    };
...
function handleAudioBufferSizeChange(val) {
  ...
  try {
    localStorage.setItem("audioFxBufferSize", String(v));
  } catch (e) {}
}
...
function _createFxScriptProcessorSafe(
  ctx,
  desiredSize,
  inChannels = 2,
  outChannels = 2,
  ...
  for (const size of candidates) {
    try {
      return ctx.createScriptProcessor(size, inChannels, outChannels);
    } catch (_) {}
  }
...
async function _audioLatencyRefresh() {
  ...
  let baseMs = NaN,
    outputMs = NaN;
  const ctx =
    typeof _pgmAudioCtx !== "undefined" && _pgmAudioCtx ? _pgmAudioCtx : null;
  if (ctx) {
    ...
    if (typeof ctx.outputLatency === "number")
      outputMs = ctx.outputLatency * 1000;
  }
  ...
  if (!Number.isFinite(inputHwMs) && Number.isFinite(baseMs))
    inputHwMs = baseMs;
  setEl("lat-val-input-hw", fmt(inputHwMs));

  // 3) FX buffer contribution
  const fxBuf =
    typeof _getAudioFxBufferSize === "function" ? _getAudioFxBufferSize() : 128;
  ...
  const inputPart = Number.isFinite(inputHwMs)
    ? inputHwMs
  ...
  const usingCustomDevice =
    typeof _pgmOutputDeviceId !== "undefined" && !!_pgmOutputDeviceId;
  const ctxSinkSupported = !!(ctx && typeof ctx.setSinkId === "function");
  const mode =
    typeof _pgmMonitoringMode !== "undefined"
      ? _pgmMonitoringMode
      : "monitor-off";
  let monitorMs = NaN;
  if (mode !== "monitor-off") {
    if (usingCustomDevice && !ctxSinkSupported) {
      ...
      monitorMs =
        basePart +
        fxBufMs +
        (Number.isFinite(outputMs) ? outputMs : basePart) +
        5;
    } else {
  ...
  if (
    ...
    if (
      ...
      Math.abs((ctx.sampleRate || preferredRate) - preferredRate) > 1
    ) {
      _pgmUltraLowLatencyState = "degraded";
    }
  }
  const ullSuffix = _pgmUltraLowLatencyEnabled
    ? ` · ${_pgmGetUllStatusText()}`
    : "";
  setEl(
    "lat-val-monitor",
    mode === "monitor-off" ? "Off" : `${fmt(monitorMs)}${ullSuffix}`,
  );
...
function switchSettingsTab(tabId) {
  const targetItem = document.querySelector(
    `.sm-sidebar-item[data-sm-tab="${tabId}"]`,
  );
...
function setFeedbackStatus(message, opts = {}) {
  ...
  if (opts.success === true && !opts.error) {
    ...
    setFeedbackSuccessOverlay(message, {
      show: true,
...
async function submitFeedbackIssue() {
  ...
  try {
    ...
    setFeedbackStatus("Feedback sent to GitHub.", {
      success: true,
      durationMs: 2200,
    });
  } catch (error) {
    const message =
      error && error.message
        ? error.message
        : "Unable to reach feedback backend.";
...
async function updateFooterBar() {
  ...
  try {
    if (
      window.BSPDesktop &&
      typeof window.BSPDesktop.getSystemStats === "function"
    ) {
      ...
      if (stats) {
        ...
        if (cpuText)
          cpuText.textContent = "CPU: " + (stats.cpu?.percent ?? 0) + "%";
        if (memText)
          memText.textContent =
            "MEM: " +
            (stats.memory?.percent ?? 0) +
            "% (" +
            formatBytes(stats.memory?.used ?? 0) +
            ")";
        if (uptimeText)
          uptimeText.textContent = formatUptime(stats.uptime ?? 0);
        if (platformText)
          platformText.textContent =
            (stats.platform === "darwin" ? "macOS" : stats.platform || "") +
            " · " +
            (stats.arch || "");
        ...
        if (verText && stats.electronVersion)
          verText.textContent = "Electron " + stats.electronVersion;
      ...
      const platformText = document.getElementById("fb-platform-text");
      if (platformText)
        platformText.textContent = navigator.platform || "Browser";
      const cpuText = document.getElementById("fb-cpu-text");
...
function setupCollapsibleSettings() {
  ...
  (function bindSettingsModalDrag() {
    ...
    titleBar.addEventListener("pointerdown", (ev) => {
      ...
      if (
        ev.target &&
        ev.target.closest("button,input,select,textarea,a,label")
      )
        return;
      _smDrag.active = true;
    ...
    const finish = (ev) => {
      ...
      try {
        titleBar.releasePointerCapture(_smDrag.pointerId);
      } catch (_) {}
...
function setupStyleEditorCollapsibles() {
  document
    .querySelectorAll("#styleEditorModal .editor-section")
    .forEach((sec) => {
      const title = sec.querySelector(".editor-section-title");
      if (!title) return;
      const content = [...sec.children].filter(
        (ch) => !ch.classList.contains("editor-section-title"),
      );
...
function applyLoadedState(
  ...
  let textRecords = [];
  if (Array.isArray(textRecordsOrOpts)) {
    textRecords = textRecordsOrOpts;
  } else {
    opts = textRecordsOrOpts || {};
  }
  ...
  bibleRecords.forEach((record) => {
    ...
    bibles[record.id] = Array.isArray(record.parsedData)
      ? record.parsedData
      : [];
  });

  appState = mergeAppStateWithDefaults(stateValue || {});
  const persistedHostMode =
    stateValue?.host?.mode ||
    (appState.settings && appState.settings.hostMode) ||
    HOST_MODE_OBS;
  const persistedVmix =
    stateValue?.host?.vmix ||
    (appState.settings && appState.settings.vmix) ||
    null;
  ...
  try {
    localPreferredLanguage = localStorage.getItem("bible_app_language") || "";
  } catch (_) {}
  const hasLocalPreferredLanguage = LANGUAGES.some(
    (entry) => entry.code === localPreferredLanguage,
  );
  const preferredLanguage = hasLocalPreferredLanguage
    ? localPreferredLanguage
    : appState.ui && appState.ui.language
      ? appState.ui.language
      : currentLanguage;
  ...
  if (ltStylesValue && typeof ltStylesValue === "object") {
    const existingLtStyles =
      appState.settings && appState.settings.ltStyles
        ? appState.settings.ltStyles
        : null;
    const mergedLtStyles = Object.assign(
      {},
      ltStylesValue,
      existingLtStyles || {},
    );
  ...
  if (
    bgSettingsValue &&
    typeof bgSettingsValue === "object" &&
    bgSettingsUpdatedAt >= stateUpdatedAt
  ) {
    appState.settings = appState.settings || {};
    applyBackgroundSnapshot(appState.settings, bgSettingsValue);
  }
  if (
    animationSettingsValue &&
    typeof animationSettingsValue === "object" &&
    animationSettingsUpdatedAt >= stateUpdatedAt
  ) {
    appState.settings = appState.settings || {};
  ...
  if (
    ...
    typeof typographySettingsValue === "object" &&
    typographySettingsUpdatedAt >= stateUpdatedAt
  ) {
    appState.settings = appState.settings || {};
  ...
  if (
    modeSettingsValue &&
    typeof modeSettingsValue === "object" &&
    modeSettingsUpdatedAt >= stateUpdatedAt
  ) {
    appState.settings = appState.settings || {};
  ...
  editorMode =
    appState.ui && appState.ui.editorMode ? appState.ui.editorMode : editorMode;
  lastLiveState =
    appState.live && appState.live.lastLiveState
      ? appState.live.lastLiveState
      : lastLiveState;
  if (appState.live) {
    ...
    liveLinesPerPage = Math.max(
      1,
      Number(appState.live.liveLinesPerPage) || Number(liveLinesPerPage) || 1,
    );
  ...
  if (appState.audioMixer) {
    ...
    _pgmMonitorVolume = Number.isFinite(rawVol)
      ? Math.max(0, Math.min(1, rawVol))
      : _pgmMonitorVolume;
    _pgmMutedMonitor = appState.audioMixer.monitorMuted !== false;
    _pgmMonitoringMode = _pgmGetNormalizedMonitoringMode(
      appState.audioMixer.monitorMode,
    );
    const rawSync = Number(appState.audioMixer.mediaAvSyncMs);
    _pgmMediaLocalAvSyncMs = Number.isFinite(rawSync)
      ? Math.max(-200, Math.min(400, Math.round(rawSync)))
      : _pgmMediaLocalAvSyncMs;
    _pgmUltraLowLatencyEnabled = appState.audioMixer.ullEnabled === true;
    _pgmUltraLowLatencyBuffer = _pgmNormalizeUllBuffer(
      appState.audioMixer.ullBuffer,
    );
    _pgmUltraLowLatencySampleRate = _pgmNormalizeUllSampleRate(
      appState.audioMixer.ullSampleRate,
    );
    _pgmUltraLowLatencyBypassFx = appState.audioMixer.ullBypassFx !== false;
    _pgmUltraLowLatencyUnderruns = Math.max(
      0,
      Math.round(Number(appState.audioMixer.ullUnderruns) || 0),
    );
    _pgmUltraLowLatencyState = _pgmUltraLowLatencyEnabled
      ? "active"
      : "inactive";
    ...
    if (typeof appState.audioMixer.masterVolume === "number") {
      _promixMasterVolume = Math.max(
        0,
        Math.min(1.5, appState.audioMixer.masterVolume),
      );
    }
    // Restore bus definitions
    if (
      Array.isArray(appState.audioMixer.buses) &&
      appState.audioMixer.buses.length
    ) {
  ...
  activeBibleVersion =
    appState.selectedBibleId ||
    (appState.bibleNav ? appState.bibleNav.versionId : null);
  if (!activeBibleVersion || !bibles[activeBibleVersion]) {
  ...
  document
    .getElementById("ratio-custom")
    .classList.toggle("active", activeRatio === "custom");
  const hasSavedBgToggle =
    appState.settings && appState.settings.bgToggle != null;
  if (activeRatio === "16-9" && !hasSavedBgToggle)
    applyLtBgDefaultForTab(sidebarTab);
  ...
  if (
    lyricButtons &&
    appState.ui &&
    appState.ui.lyricButtonsScrollTop != null
  ) {
  ...
  if (monitorBtn) {
    ...
    monitorBtn.title = _pgmMutedMonitor
...
async function bootApp() {
  ...
  const [
    ...
    ltStylesEntry,
    bgSettingsEntry,
    animationSettingsEntry,
    typographySettingsEntry,
    modeSettingsEntry,
  ] = await Promise.all([
  ...
  applyLoadedState(
    ...
    bibleRecords,
    {
      ...
      animationSettingsValue: animationSettingsEntry
        ? animationSettingsEntry.value
        : null,
      animationSettingsUpdatedAt: animationSettingsEntry
        ? animationSettingsEntry.updatedAt || 0
        : 0,
      typographySettingsValue: typographySettingsEntry
        ? typographySettingsEntry.value
        : null,
      typographySettingsUpdatedAt: typographySettingsEntry
        ? typographySettingsEntry.updatedAt || 0
        : 0,
      modeSettingsValue: modeSettingsEntry ? modeSettingsEntry.value : null,
      modeSettingsUpdatedAt: modeSettingsEntry
        ? modeSettingsEntry.updatedAt || 0
        : 0,
  ...
  if (
    isVmixMode() &&
    vmixState.enabled &&
    vmixState.reconnectOnStartup !== false
  ) {
    vmixReconnect().catch(() => {});
  }
}
...
function applyObsCompatibilityUi() {
  const obsSafeMode =
    typeof shouldUseObsBrowserSafeMode === "function"
  ...
  const standaloneOption = hostModeSelect
    ? Array.from(hostModeSelect.options || []).find(
        (opt) => opt.value === HOST_MODE_STANDALONE,
      )
    : null;
  if (standaloneOption) {
  ...
  if (
    obsSafeMode &&
    hostModeSelect &&
    hostModeSelect.value === HOST_MODE_STANDALONE
  ) {
