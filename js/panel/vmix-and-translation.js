function getRuntimeHostModeOverride() {
  try {
    ...
    const requested = String(params.get("hostMode") || "")
      .trim()
      .toLowerCase();
    if (
      [HOST_MODE_OBS, HOST_MODE_VMIX, HOST_MODE_STANDALONE].includes(requested)
    ) {
      return requested;
    }
  } catch (_) {}
  ...
  if (
    ...
    typeof hasDesktopBridgeRuntime === "function" &&
    !hasDesktopBridgeRuntime()
  ) {
    return HOST_MODE_OBS;
  }
  return "";
...
function getHostMode() {
  ...
  return [HOST_MODE_OBS, HOST_MODE_VMIX, HOST_MODE_STANDALONE].includes(
    hostMode,
  )
    ? hostMode
    : HOST_MODE_OBS;
...
function isVmixAdvancedMode() {
  return (
    vmixState.outputMode !== VMIX_OUTPUT_MODE_DEDICATED ||
...
function updateVmixStatusUi() {
  ...
  if (bodyEl)
    bodyEl.style.display = panel?.classList.contains("is-collapsed")
      ? "none"
      : "flex";
  if (toggleEl)
    toggleEl.textContent = panel?.classList.contains("is-collapsed")
      ? ">"
      : "v";
  if (connectionEl) {
    if (!vmixState.enabled) connectionEl.textContent = "vMix disabled";
    else if (vmixConnectionState === "connected")
      connectionEl.textContent = "vMix connected";
    else if (vmixConnectionState === "connecting")
      connectionEl.textContent = "Connecting to vMix...";
    else if (vmixConnectionState === "error")
      connectionEl.textContent = `vMix error: ${vmixLastError || "Unknown error"}`;
    else connectionEl.textContent = "vMix disconnected";
  }
  if (summaryEl)
    summaryEl.textContent = connectionEl
  ...
  if (modeEl) {
    modeEl.textContent = isVmixAdvancedMode()
      ? "Advanced routing"
      : "Basic routing";
  }
  if (urlEl) {
...
async function refreshLocalServerInfo() {
  if (
    !(
      window.BSPDesktop &&
      typeof window.BSPDesktop.getLocalServerInfo === "function"
    )
  )
    return null;
  try {
...
function getVmixDisplayUrl() {
  if (localServerInfo) {
    const selectedHost =
      String(vmixState.displayHost || "").trim() ||
      localServerInfo.preferredHost ||
      "127.0.0.1";
...
function vmixParseInputs(xmlText) {
  ...
  return Array.from(doc.querySelectorAll("vmix > inputs > input")).map(
    (node) => ({
  ...
  return (
    inputs.find((input) => {
      const title = String(input.title || "")
        .trim()
        .toLowerCase();
      const shortTitle = String(input.shortTitle || "")
        .trim()
        .toLowerCase();
...
function vmixResolveConfiguredOutputInput(inputs = []) {
  const byKey = vmixState.outputInputKey
    ? inputs.find(
        (input) =>
          String(input.key || "") === String(vmixState.outputInputKey || ""),
      )
    : null;
...
async function vmixConnect() {
  ...
  if (
    !String(vmixState.host || "").trim() ||
    !String(vmixState.port || "").trim()
  ) {
    vmixConnectionState = "error";
  ...
  try {
    ...
    if (
      vmixResolvedInput?.key &&
      vmixResolvedInput.key !== vmixState.outputInputKey
    ) {
      updateVmixSettings(
        { outputInputKey: vmixResolvedInput.key },
        { silent: true },
      );
    }
    ...
    vmixLastError =
      error && error.message ? error.message : "Unable to reach vMix API";
    updateVmixStatusUi();
    return false;
  }
}
...
async function vmixEnsureOutputInput() {
  if (vmixResolvedInput?.key || vmixResolvedInput?.title)
    return vmixResolvedInput;
...
async function vmixHideOutput() {
  ...
  if (vmixState.outputMode === VMIX_OUTPUT_MODE_DEDICATED) {
    showToast(
      "Dedicated input mode does not auto-hide because vMix may already be on another shot",
    );
    return true;
  }
  showToast("Manual mode leaves show/hide control to the vMix operator");
...
async function handleVmixShowAction() {
  try {
    ...
    showToast(
      error && error.message ? error.message : "Unable to show in vMix",
    );
  }
}
...
async function handleVmixHideAction() {
  try {
    ...
    showToast(
      error && error.message ? error.message : "Unable to hide in vMix",
    );
  }
}
...
function setHostMode(mode, opts = {}) {
  ...
  hostMode =
    runtimeOverride ||
    ([HOST_MODE_OBS, HOST_MODE_VMIX, HOST_MODE_STANDALONE].includes(mode)
      ? mode
      : HOST_MODE_OBS);
  applyHostModeUi();
...
function restoreVmixSettingsUi() {
  ...
  if (outputModeEl)
    outputModeEl.value = vmixState.outputMode || VMIX_OUTPUT_MODE_DEDICATED;
...
function getSongBilingualSettings() {
  ...
  return {
    ...
    displayMode: ["primary", "stacked", "translated-only"].includes(
      ui.songDisplayMode,
    )
      ? ui.songDisplayMode
      : DEFAULT_SONG_BILINGUAL_SETTINGS.displayMode,
    translationMode: ["free", "premium", "manual"].includes(
      ui.songTranslationMode,
    )
      ? ui.songTranslationMode
...
function populateSongTranslationLanguageOptions() {
  ...
  const sourceSelect = document.getElementById(
    "song-translation-source-language",
  );
  if (targetSelect && !targetSelect.dataset.populated) {
    const targetOptions = SONG_TRANSLATION_LANGUAGES.filter(
      (lang) => lang.code !== "auto",
    )
  ...
  if (sourceSelect && !sourceSelect.dataset.populated) {
    sourceSelect.innerHTML = SONG_TRANSLATION_LANGUAGES.map(
...
function updateSongTranslationModeUi() {
  ...
  const premiumFields = document.getElementById(
    "song-translation-premium-fields",
  );
  if (freeFields) freeFields.style.display = mode === "free" ? "flex" : "none";
  if (premiumFields)
    premiumFields.style.display = mode === "premium" ? "flex" : "none";
}
...
function computeTranslationHash(text, lang) {
  return `${String(lang || "")
    .trim()
    .toLowerCase()}::${normalizeSongLyricsLineBreaks(text || "").trim()}`;
}

function normalizeSongTranslationState(song) {
  if (!song || getIsBibleItem(song) || isTextItem(song)) return song;
  const settings = getSongBilingualSettings();
  if (typeof song.translatedLyrics !== "string") song.translatedLyrics = "";
  if (
    typeof song.translationLanguage !== "string" ||
  ...
  if (typeof song.translationHash !== "string" || !song.translationHash) {
    song.translationHash = computeTranslationHash(
      content,
      song.translationLanguage,
    );
  }
...
function markSongTranslationStale(song) {
  if (!song || getIsBibleItem(song) || isTextItem(song)) return;
...
function songNeedsTranslation(song, lang) {
  if (!song || getIsBibleItem(song) || isTextItem(song)) return false;
  const targetLanguage =
    String(lang || "").trim() || getSongBilingualSettings().targetLanguage;
  const nextHash = computeTranslationHash(
...
function getSongTranslationStatusLabel(song) {
  ...
  if (status === "ready")
    return `Ready${song.translationLanguage ? ` (${song.translationLanguage})` : ""}`;
...
function resolveSongTranslationProviderConfig(opts = {}) {
  ...
  if (mode === "manual") {
    return {
      ...
      sourceLanguage:
        String(
          opts.sourceLanguage || settings.sourceLanguage || "auto",
        ).trim() || "auto",
      targetLanguage:
        String(opts.targetLanguage || settings.targetLanguage || "fr").trim() ||
        "fr",
  ...
  const url =
    mode === "free"
      ? String(
          opts.translationApiUrl || settings.freeTranslationApiUrl || "",
        ).trim()
      : String(
          opts.translationApiUrl || settings.translationApiUrl || "",
        ).trim();
  return {
    mode,
    available: !!url,
    ...
    apiKey:
      mode === "premium"
        ? String(
            opts.translationApiKey || settings.translationApiKey || "",
          ).trim()
        : "",
    sourceLanguage:
      String(opts.sourceLanguage || settings.sourceLanguage || "auto").trim() ||
      "auto",
    targetLanguage:
      String(opts.targetLanguage || settings.targetLanguage || "fr").trim() ||
      "fr",
    reason:
      mode === "free"
        ? "Free translation URL is not configured"
        : "Premium Translation API URL is not configured",
  };
}
...
function refreshSongTranslationPanel(song = currentItem) {
  ...
  const bilingualToggle = document.getElementById(
    "song-bilingual-item-enabled",
  );
  const lockToggle = document.getElementById("song-translation-lock");
  if (!panel || !statusEl || !textarea || !bilingualToggle || !lockToggle)
    return;
  const isSong = !!(song && !getIsBibleItem(song) && !isTextItem(song));
...
function setGlobalSongDisplayMode(mode) {
  const nextMode = ["primary", "stacked", "translated-only"].includes(mode)
    ? mode
    : DEFAULT_SONG_BILINGUAL_SETTINGS.displayMode;
  if (!appState) appState = buildDefaultAppState();
  if (!appState.settings) appState.settings = getUiSnapshot();
...
function handleSongSecondaryLyricsInput() {
  if (!currentItem || getIsBibleItem(currentItem) || isTextItem(currentItem))
    return;
  normalizeSongTranslationState(currentItem);
  currentItem.translatedLyrics =
    document.getElementById("song-translation-editor")?.value || "";
  currentItem.translationLanguage = getSongBilingualSettings().targetLanguage;
  currentItem.translationHash = computeTranslationHash(
    currentItem.content || currentItem.text || "",
    currentItem.translationLanguage,
  );
  currentItem.translationStatus = currentItem.translatedLyrics.trim()
    ? "ready"
    : "idle";
  currentItem.translatedAt = currentItem.translatedLyrics.trim()
    ? Date.now()
    : 0;
...
function handleSongBilingualToggleChange() {
  setGlobalSongBilingualEnabled(
    !!document.getElementById("song-bilingual-item-enabled")?.checked,
  );
}
...
function handleSongTranslationLockChange() {
  ...
  currentItem.translationLocked = !!document.getElementById(
    "song-translation-lock",
  )?.checked;
  scheduleSongPersist(currentItem);
  saveState();
  saveToStorageDebounced();
...
async function translateTextBlocksFree(text, provider) {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      errorText || `Translation request failed (${response.status})`,
    );
  }
  const data = await response.json();
  const translatedJoined = parseGoogleTranslateText(data);
  if (!translatedJoined)
    throw new Error("Free translation returned no translated text");
  const translatedLines = translatedJoined.split(/\s\[-\]\s/g);
  const normalized = lines
    .map((_, index) =>
      translatedLines[index] != null ? translatedLines[index] : "",
    )
    .join("\n");
  return restoreSongTranslationMarkers(
    normalizeSongLyricsLineBreaks(normalized),
    markers,
  );
}
...
async function translateTextBlocksPremium(text, provider) {
  ...
  return restoreSongTranslationMarkers(
    normalizeSongLyricsLineBreaks(translatedText),
    markers,
  );
}
...
async function testSongTranslationProvider() {
  ...
  try {
    const sample = await translateTextBlocks(
      "Hello world",
    ...
    setSongTranslationProviderStatus(
      `${provider.mode === "free" ? "Free" : "Premium"} connected: ${sample}`,
      "success",
    );
    showToast(
      `${provider.mode === "free" ? "Free" : "Premium"} translation provider connected`,
    );
    return true;
  } catch (error) {
    const message =
      error && error.message ? error.message : "Connection test failed";
...
async function translateSong(song, opts = {}) {
  ...
  const settings = getSongBilingualSettings();
  const lang = String(
    opts.targetLanguage || settings.targetLanguage || "fr",
  ).trim();
  ...
  refreshSongTranslationPanel(song);
  try {
    const translatedLyrics = await translateTextBlocks(
      song.content || song.text || "",
      lang,
      {
        ...opts,
...
function maybeTranslateCurrentSongOnOpen() {
  ...
  if (
    ...
    songNeedsTranslation(currentItem, settings.targetLanguage)
  ) {
...
function maybeTranslateImportedSong(song) {
  ...
  normalizeSongTranslationState(song);
  if (
    !song.translationLocked &&
    songNeedsTranslation(song, settings.targetLanguage)
  ) {
