...
function saveWorkspaceTabPreference(tabId) {
  try {
    localStorage.setItem("activeWorkspaceTab", String(tabId || "bible"));
  } catch (e) {}
}
...
function restoreWorkspaceTabPreference() {
  ...
  try {
    saved = String(
      localStorage.getItem("activeWorkspaceTab") || "",
    ).toLowerCase();
  } catch (e) {}
  if (!saved) return;
  if (saved === "song" || saved === "songs") {
    setDockSceneTab("song");
    return;
  }
  if (saved === "text") {
    setSidebarTab("text");
...
const SIDEBAR_QUICK_ACTION_GROUPS = [
  {
    ...
    items: [
      {
        id: "show-song-category-name",
        labelKey: "settings_show_category_name",
      },
      {
        id: "display-song-sections",
        labelKey: "settings_display_songs_by_sections",
      },
      {
  ...
  },
  {
    ...
    items: [
      {
        id: "shorten-bible-versions",
        labelKey: "settings_shorten_bible_versions",
      },
      { id: "shorten-bible-books", labelKey: "settings_shorten_bible_books" },
      {
        id: "version-switch-updates-live",
        labelKey: "settings_version_switch_updates_output",
      },
    ],
  },
];
...
function saveCurrentItem() {
  ...
  if (sidebarTab === "text") {
    ...
    currentItem.searchableText = normalizeSearchText(
      `${currentItem.title || ""}\n${nextText}`,
    );
    scheduleTextPersist(currentItem);
  } else {
    ...
    if (nextText !== prevText) {
      ...
      if (
        ...
        settings.autoTranslateOnOpen &&
        !currentItem.translationLocked
      ) {
  ...
  if (
    isLive &&
    livePointer &&
    livePointer.kind === "songs" &&
    livePointer.index === currentIndex
  )
    scheduleLiveUpdate();
}
...
function captureSingleLineFontBaseline() {
  ...
  if (!fontInput) return;
  singleLineFontSizeSnapshot = Number(
    fontInput.value || DEFAULT_BIBLE_FULL_FONT,
  );
}
...
function createDefaultFocusedWorkspaceControlState(tab) {
  ...
  return {
    linesPerPage:
      tab === "bible" ? 1 : Math.min(2, getMaxLinesForCurrentTab(tab)),
    activeRatio: "full",
...
function normalizeFocusedWorkspaceBackgroundByRatio(raw, legacy, tab) {
  const defaults =
    createDefaultFocusedWorkspaceControlState(tab).backgroundByRatio;
  const source = raw && typeof raw === "object" ? raw : {};
  const result = {
    full: normalizeFocusedWorkspaceBackgroundState(
      source.full || defaults.full,
    ),
...
function normalizeFocusedWorkspaceControlState(raw, tab) {
  ...
  const backgroundByRatio = normalizeFocusedWorkspaceBackgroundByRatio(
    next.backgroundByRatio,
    next,
    tab,
  );
  const backgroundLinked =
    next.backgroundLinked == null
      ? base.backgroundLinked
      : next.backgroundLinked === true;
  let activeBackground =
    backgroundByRatio[getFocusedWorkspaceRatioKey(rawRatio)] ||
    backgroundByRatio.full;
  if (backgroundLinked) {
    activeBackground =
      normalizeFocusedWorkspaceBackgroundState(activeBackground);
  ...
  return {
    linesPerPage: Math.max(
      ...
      Math.min(
        maxLines,
        Number.isFinite(rawLines) ? rawLines : base.linesPerPage,
      ),
    ),
    ...
    backgroundByRatio,
    bgType:
      next.bgType === "image" || next.bgType === "video"
        ? next.bgType
        : "color",
    songTransitionType: String(
      next.songTransitionType || base.songTransitionType || "fade",
    ),
    songTransitionDuration: String(
      next.songTransitionDuration || base.songTransitionDuration || "0.8",
    ),
    animateBgTransitions:
      next.animateBgTransitions == null
        ? base.animateBgTransitions
...
function ensureFocusedWorkspaceControlsState() {
  if (
    ...
    typeof focusedWorkspaceControlsByTab !== "object"
  ) {
    focusedWorkspaceControlsByTab = createDefaultFocusedWorkspaceControls();
    return focusedWorkspaceControlsByTab;
  }
  FOCUSED_WORKSPACE_TABS.forEach((tab) => {
    focusedWorkspaceControlsByTab[tab] = normalizeFocusedWorkspaceControlState(
...
function captureFocusedWorkspaceControlState(tab = sidebarTab) {
  ...
  const existing =
    focusedWorkspaceControlsByTab && focusedWorkspaceControlsByTab[tab]
      ? focusedWorkspaceControlsByTab[tab]
      : null;
  const backgroundByRatio = normalizeFocusedWorkspaceBackgroundByRatio(
    existing && existing.backgroundByRatio,
    existing,
    tab,
  );
  ...
  backgroundByRatio[getFocusedWorkspaceRatioKey(activeRatio)] =
    currentBackground;
  if (backgroundLinked) {
  ...
  return normalizeFocusedWorkspaceControlState(
    {
      ...
      songTransitionType:
        document.getElementById("song-transition-type")?.value || "fade",
      songTransitionDuration:
        document.getElementById("song-transition-duration")?.value || "0.8",
      animateBgTransitions:
        document.getElementById("animate-bg-transitions")?.checked !== false,
      presetPopoverOpen:
        typeof presetPopoverOpen !== "undefined" ? !!presetPopoverOpen : false,
      ...(typeof getAnnotateWorkspaceState === "function"
        ? getAnnotateWorkspaceState()
        : {}),
    },
    tab,
  );
...
function loadFocusedWorkspaceControlsFromUi(ui) {
  ...
  const source =
    ...
    typeof ui.focusedWorkspaceControls === "object"
      ? ui.focusedWorkspaceControls
      : {};
  focusedWorkspaceControlsByTab = {};
  FOCUSED_WORKSPACE_TABS.forEach((tab) => {
    focusedWorkspaceControlsByTab[tab] = normalizeFocusedWorkspaceControlState(
      source[tab] || defaults[tab],
      tab,
    );
  });
  return focusedWorkspaceControlsByTab;
...
function applyFocusedWorkspaceControlsForTab(tab, opts = {}) {
  ...
  if (ratioFullBtn)
    ratioFullBtn.classList.toggle("active", activeRatio === "full");
  ...
  handleBgTypeChange();
  syncBgOpacitySlider();
  if (typeof setPresetPopoverOpen === "function")
    setPresetPopoverOpen(!!next.presetPopoverOpen, { persist: false });
...
function applyFocusedWorkspaceBackgroundForRatio(
  tab = sidebarTab,
  ratio = activeRatio,
) {
  if (!FOCUSED_WORKSPACE_TABS.includes(tab)) return false;
  ...
  controls[tab] = state;
  const next =
    state.backgroundByRatio[getFocusedWorkspaceRatioKey(ratio)] ||
    state.backgroundByRatio.full;
  bgMode = next.bgMode;
  const bgToggle = document.getElementById("bg-toggle");
...
function updateModeBackgroundLinkUi() {
  ...
  icon.title = linked
    ? "Unlink FS and LT backgrounds"
    : "Link FS and LT backgrounds";
...
function setModeBackgroundLinked(linked) {
  ...
  const controls = ensureFocusedWorkspaceControlsState();
  const state = normalizeFocusedWorkspaceControlState(
    controls[sidebarTab],
    sidebarTab,
  );
  const bgToggle = document.getElementById("bg-toggle");
  ...
  state.backgroundByRatio[getFocusedWorkspaceRatioKey(activeRatio)] = {
    ...currentBackground,
  };
  ...
  controls[sidebarTab] = normalizeFocusedWorkspaceControlState(
    state,
    sidebarTab,
  );
  updateModeBackgroundLinkUi();
...
function getWorkspaceFontSizeValue() {
  ...
  const fallback =
    activeRatio === "16-9"
      ? typeof getEffectiveLtFont === "function"
        ? getEffectiveLtFont()
        : 36
...
function stepWorkspaceFontSize(direction) {
  ...
  const delta =
    (Number.isFinite(step) && step > 0 ? step : 1) * (Number(direction) || 0);
...
function getMaxLinesForCurrentTab(tab = sidebarTab) {
  return tab === "songs" || tab === "text" ? 6 : 3;
}

function updateLinePickerAvailability() {
  const isSongs = sidebarTab === "songs" || sidebarTab === "text";
...
function setLines(n, opts = {}) {
  ...
  const isBible =
    sidebarTab === "bible" && currentItem && getIsBibleItem(currentItem);
  ...
  if (isBible && !anchorVerse) {
    ...
    anchorVerse =
      prevPage && prevPage.startVerse
        ? String(prevPage.startVerse).trim()
        : getFirstVerseNumber(prevPage && prevPage.raw);
  }
  ...
  if (isBible && anchorVerse) {
    ...
    const targetIdx = pages.findIndex((p) =>
      matchesVerseStart(p.raw, anchorVerse),
    );
    if (targetIdx !== -1) lineCursor = targetIdx;
  }
  document
    .querySelectorAll("#line-picker .seg-btn")
    .forEach((b) => b.classList.toggle("active", b.id === "line-" + n));
  updateButtonView();
  if (
    isLive &&
    livePointer &&
    ...
    currentItem &&
    currentItem.version === livePointer.version &&
    currentIndex === livePointer.index
  ) {
...
function updateLtBibleVerseAlignButtons() {
  const hMap = {
    left: "lt-bv-h-left",
    center: "lt-bv-h-center",
    right: "lt-bv-h-right",
    justify: "lt-bv-h-justify",
  };
  Object.entries(hMap).forEach(([key, id]) => {
    const el = document.getElementById(id);
...
function updateFullAlignButtons() {
  const refMap = {
    left: "full-ref-h-left",
    center: "full-ref-h-center",
    right: "full-ref-h-right",
    justify: "full-ref-h-justify",
  };
  ...
  const hMap = {
    left: "full-h-left",
    center: "full-h-center",
    right: "full-h-right",
    justify: "full-h-justify",
  };
  Object.entries(hMap).forEach(([key, id]) => {
    const el = document.getElementById(id);
  ...
  const vMap = {
    top: "full-v-top",
    middle: "full-v-middle",
    bottom: "full-v-bottom",
  };
  Object.entries(vMap).forEach(([key, id]) => {
    const el = document.getElementById(id);
...
function updateBarAlignButtons() {
  ...
  const hMap = {
    left: "se-bar-h-left",
    center: "se-bar-h-center",
    right: "se-bar-h-right",
    justify: "se-bar-h-justify",
  };
  Object.entries(hMap).forEach(([key, id]) => {
    const el = document.getElementById(id);
  ...
  const vMap = {
    top: "se-bar-v-top",
    middle: "se-bar-v-middle",
    bottom: "se-bar-v-bottom",
  };
  Object.entries(vMap).forEach(([key, id]) => {
    const el = document.getElementById(id);
...
function updateRefAlignButtons() {
  ...
  const hMap = {
    left: "se-ref-h-left",
    center: "se-ref-h-center",
    right: "se-ref-h-right",
    justify: "se-ref-h-justify",
  };
  Object.entries(hMap).forEach(([key, id]) => {
    const el = document.getElementById(id);
    if (el) el.classList.toggle("active", hVal === key);
  ...
  const vMap = {
    top: "se-ref-v-top",
    middle: "se-ref-v-middle",
    bottom: "se-ref-v-bottom",
  };
  Object.entries(vMap).forEach(([key, id]) => {
    const el = document.getElementById(id);
    if (el) el.classList.toggle("active", vVal === key);
...
function setRefBgEnabled(enabled, opts = {}) {
  ...
  const effectiveTarget =
    typeof getEffectiveSettingsTargetTab === "function"
      ? getEffectiveSettingsTargetTab()
      : "bible";
  if (row)
    row.style.display =
      effectiveTarget === "songs" ? "none" : refBgEnabled ? "" : "none";
  if (!opts.silent && typeof onAnyControlChange === "function")
    onAnyControlChange();
}
...
function shouldApplyDualFontOverride() {
  const autoResizeFull =
    document.getElementById("auto-resize-full")?.value || "none";
  return (
...
function restoreDualFontOverride(force = false, opts = {}) {
  ...
  if (!opts.suppressLiveUpdate && typeof onAnyControlChange === "function")
    onAnyControlChange();
}
...
function applyDualLtOverride(opts = {}) {
  ...
  const dualBibleLtMaxFont =
    typeof DUAL_BIBLE_LT_MAX_FONT === "number" ? DUAL_BIBLE_LT_MAX_FONT : 23;
  const nextVerseSize = Math.min(
    Number(verseInput.value || dualBibleLtMaxFont),
    dualBibleLtMaxFont,
  );
...
function applyLongVerseFullFontOverride(
  key,
  factor = 0.8,
  sourceFontSize = null,
) {
  ...
  if (longVerseFullFontActive) {
    if (longVerseFullVerseKey === key && longVerseFullFontFactor === factor)
      return;
    restoreLongVerseFullFontOverride(true);
  }
  if (longVerseFullFontSnapshot == null) {
    const snapshot = Number.isFinite(sourceFontSize)
      ? sourceFontSize
      : captureFullFontSize();
...
function handleLongVerseFullFontState(
  shouldApply,
  key,
  reductionFactor = 0.8,
  sourceFontSize = null,
...
function updateDualVersionSelects() {
  ...
  if (primarySelect) {
    ...
    if (versions.length) {
      primarySelect.value =
        activeBibleVersion && versions.includes(activeBibleVersion)
          ? activeBibleVersion
          : versions[0];
    } else {
      primarySelect.value = "";
...
function buildReferenceBadgeSpan(
  text,
  fontSizePt,
  foregroundColor,
  backgroundColor,
  ...
  const allowBackground =
    opts.allowBackground !== undefined ? opts.allowBackground : true;
...
function buildFullBibleSegment({
  referenceLabel,
  verseHtml,
  refSize,
  refColor,
  ...
  const normalizedAlign = ["left", "right", "center"].includes(refAlign)
    ? refAlign
    : "center";
  ...
  const refBadge = buildReferenceBadgeSpan(
    ...
    refColor,
    refBgColor,
    { textTransform: refTextTransform || fullRefTextTransform },
  );
  ...
  return refPosition === "bottom"
    ? bodyHtml + spacerHtml + refHtml
    : refHtml + bodyHtml;
}

function buildLtBibleSegment({
  referenceLabel,
  verseHtml,
  refSize,
  refColor,
  ...
  const refBadge = buildReferenceBadgeSpan(
    ...
    refBgColor,
    {
      ...
      padding,
      borderRadius,
      textTransform: refTextTransform || ltRefTextTransform,
    },
  );
  return (
    `<div class="jo-ref-line" style="margin-bottom:3px;width:100%;text-align:${alignValue}">${refBadge}</div>` +
...
function setEditorMode(m) {
  ...
  document.getElementById("lyric-editor").style.display =
    m === "text" ? "block" : "none";
  document.getElementById("lyric-buttons").style.display =
    m === "btn" ? "block" : "none";
  const translationPanel = document.getElementById("song-translation-panel");
  if (translationPanel) {
    const showTranslationPanel =
      m === "text" && !!(currentItem && !getIsBibleItem(currentItem));
...
function insertIntoSchedule(
  entry,
  { successMessage = null, duplicateMessage = null } = {},
) {
  ...
  if (key) {
    const existingIdx = schedule.findIndex(
      (item) => getScheduleEntryKey(item) === key,
    );
    if (existingIdx !== -1) {
...
function addToSet(i, e) {
  ...
  insertIntoSchedule(
    { ...list[i] },
    {
      ...
      duplicateMessage:
        "This entry already exists on the setlist; moved to the top",
    },
  );
}
...
function deleteItem(i, e) {
  ...
  if (sidebarTab === "songs") {
    ...
    if (removed && removed.id)
      idbDelete(STORE_SONGS, removed.id).catch(() => {});
  } else if (sidebarTab === "text") {
    const removed = texts.splice(i, 1)[0];
    if (removed && removed.id)
      idbDelete(STORE_TEXTS, removed.id).catch(() => {});
...
function renameSongTitle(songIndex, nextTitleRaw) {
  ...
  if (nextKey !== currentKey) {
    const exists = songs.some(
      (s, i) => i !== idx && normalizeSongTitleKey(s?.title) === nextKey,
    );
    if (exists) {
  ...
  song.searchableText = normalizeSearchText(
...
function renameTextTitle(textIndex, nextTitleRaw) {
  ...
  const exists = texts.some(
    (item, i) =>
      i !== idx &&
      normalizeSearchText(item?.title || "") === normalizeSearchText(nextTitle),
  );
  if (exists) {
    showToast("Text title already exists");
    return false;
  }
  textDoc.title = nextTitle;
  textDoc.updatedAt = Date.now();
  textDoc.searchableText = normalizeSearchText(
    `${textDoc.title}\n${textDoc.content || textDoc.text || ""}`,
  );
...
function openModal(id) {
  ...
  if (id === "settingsModal") {
    ...
    const hasSavedAutoLyricsSearch = !!(
      (document.getElementById("auto-lyrics-title")?.value || "").trim() ||
      autoLyricsResults.length
    );
    updateAutoLyricsStatus(
      hasSavedAutoLyricsSearch
        ? autoLyricsResults.length
          ? t("auto_lyrics_found_results_click_add").replace(
              "{count}",
              String(autoLyricsResults.length),
            )
          : t("auto_lyrics_continue_last_search_or_new")
        : t("auto_lyrics_prompt"),
      hasSavedAutoLyricsSearch && autoLyricsResults.length
        ? "success"
        : "muted",
    );
...
function setNewSongLookupBusy(busy) {
  ...
  btn.textContent = newSongLookupBusy
    ? t("common_searching")
    : t("new_song_search_lyrics");
}
...
function renderNewSongLookupResults() {
  ...
  newSongLookupResults.forEach((entry, idx) => {
    ...
    btn.type = "button";
    btn.style.cssText =
      "width:100%;text-align:left;border:1px solid var(--border);background:rgba(255,255,255,0.02);color:var(--text);padding:8px 10px;border-radius:7px;margin:0 0 6px;cursor:pointer;";
...
function extractLyricsFromLookupPayload(payload) {
  ...
  for (const c of candidates) {
    if (typeof c === "string" && c.trim())
      return inferSectionedLyricsFromText(c);
  }
...
function normalizeLyricsSearchResultEntry(entry) {
  ...
  const title = String(
    entry.trackName || entry.track_name || entry.title || entry.name || "",
  ).trim();
  const artist = String(
    entry.artistName || entry.artist_name || entry.artist || "",
  ).trim();
  const album = String(
    entry.albumName || entry.album_name || entry.album || "",
  ).trim();
...
function selectNewSongLookupResult(index) {
  ...
  const lyrics =
    picked && picked.lyrics
      ? picked.lyrics
      : extractLyricsFromLookupPayload(picked);
  if (!lyrics) {
  ...
  newSongFetchedLookupKey = getNewSongLookupKey(
    titleInput?.value || "",
    artistInput?.value || "",
  );
...
async function autoFetchNewSongLyrics() {
  ...
  if (!title && !artist) {
    ...
    updateNewSongFetchStatus(
      t("auto_lyrics_enter_title_or_artist_first"),
      "error",
    );
    return;
  }
  ...
  try {
    ...
    updateNewSongFetchStatus(
      t("new_song_found_results_choose_one").replace(
        "{count}",
        String(newSongLookupResults.length),
      ),
      "muted",
    );
...
function setAutoLyricsLookupBusy(busy) {
  ...
  btn.textContent = autoLyricsLookupBusy
    ? t("common_searching")
    : t("common_search");
}
...
function updateAutoLyricsInputClearButtons() {
  ...
  if (titleClear)
    titleClear.style.display = titleInput && titleInput.value ? "flex" : "none";
  if (artistClear)
    artistClear.style.display =
      artistInput && artistInput.value ? "flex" : "none";
}

function clearAutoLyricsField(kind) {
  const input = document.getElementById(
    kind === "artist" ? "auto-lyrics-artist" : "auto-lyrics-title",
  );
  if (!input) return;
  input.value = "";
  updateAutoLyricsInputClearButtons();
...
function bindAutoLyricsGlobalHandlers() {
  ...
  document.addEventListener("mousedown", (e) => {
    ...
    if (
      ...
      t.closest("#auto-lyrics-context-menu")
    )
      return;
    closeAutoLyricsContextMenu();
  });
...
function showAutoLyricsPreview(entry, clientX, clientY) {
  ...
  const body = String(entry.lyrics || "")
    .trim()
    .split("\n")
    .slice(0, 24)
    .join("\n");
  ...
  if (left + w > window.innerWidth - pad)
    left = Math.max(pad, (clientX || 0) - w - 12);
...
function buildUniqueSongTitle(baseTitle) {
  ...
  const used = new Set(
    songs.map((s) => normalizeSongTitleKeyForInsert(s && s.title)),
  );
...
function addAutoLyricsResultToSongs(entry, opts = {}) {
  ...
  showToast(
    openEditor
      ? t("auto_lyrics_added_and_opened")
      : t("auto_lyrics_added_to_songs"),
  );
  return newSong;
}
...
function openAutoLyricsResultContextMenu(entry, x, y) {
  ...
  addBtn.style.cssText =
    "display:block;width:100%;text-align:left;background:transparent;border:none;color:var(--text);padding:8px 10px;border-radius:6px;cursor:pointer;";
  addBtn.textContent = t("auto_lyrics_add_to_songs");
  addBtn.onclick = () => {
    if (autoLyricsContextEntry)
      addAutoLyricsResultToSongs(autoLyricsContextEntry, {
        openEditor: false,
        closeModal: false,
      });
    closeAutoLyricsContextMenu();
...
function renderAutoLyricsResults() {
  ...
  autoLyricsResults.forEach((entry) => {
    ...
    row.type = "button";
    row.style.cssText =
      "display:block;width:100%;text-align:left;background:rgba(255,255,255,0.02);border:1px solid var(--border);border-radius:8px;padding:8px 10px;margin:0 0 8px;color:var(--text);cursor:pointer;";
    ...
    row.onclick = () => {
      addAutoLyricsResultToSongs(entry, {
        openEditor: true,
        closeModal: false,
      });
    };
    ...
    row.onmouseenter = (e) =>
      showAutoLyricsPreview(entry, e.clientX, e.clientY);
...
async function searchAutoLyrics() {
  ...
  const title = (
    document.getElementById("auto-lyrics-title")?.value || ""
  ).trim();
  const artist = (
    document.getElementById("auto-lyrics-artist")?.value || ""
  ).trim();
  if (!title && !artist) {
    updateAutoLyricsStatus(
      t("auto_lyrics_enter_title_or_artist_first"),
...
function updateSidebarToggleTooltip() {
  ...
  if (isSidebarDockedLayoutActive()) {
    const title = s.classList.contains("sidebar-open")
      ? "close sidebar"
      : "open sidebar";
  ...
  const itemsHtml = entries.length
    ? entries
        .map((entry, idx) => {
          ...
          const previewSource = String(entry?.content || entry?.text || "")
            .replace(/\[[^\]]+\]/g, " ")
            .replace(/\s+/g, " ")
            .trim();
        ...
        .join("")
    : `<div class="focused-main-empty">Setlist is empty. Add songs or Bible verses from the sidebar and they will appear here.</div>`;
...
function refreshWorkspaceLayoutUi() {
  ...
  if (editorContainer)
    editorContainer.style.display = isScheduleFocused ? "none" : "flex";
  if (focusedPanel)
    focusedPanel.style.display = isScheduleFocused ? "flex" : "none";
...
function applyWorkspaceLayoutMode(mode, opts = {}) {
  ...
  if (select && select.value !== workspaceLayoutMode)
    select.value = workspaceLayoutMode;
  refreshWorkspaceLayoutUi();
...
function handleWorkspaceLayoutModeChange() {
  const select = document.getElementById("workspace-layout-mode-select");
  applyWorkspaceLayoutMode(select ? select.value : "classic", {
    persist: true,
  });
}
...
function updateFocusedWorkspaceToolbar() {
  ...
  if (modeSwitch)
    modeSwitch.style.display = !focused || !isScheduleTab ? "" : "none";
  if (importBtn) importBtn.style.display = !focused || isSongsTab ? "" : "none";
  if (bibleToolWrap)
    bibleToolWrap.style.display = !focused || isBibleTab ? "" : "none";
  if (annotateWrap)
    annotateWrap.style.display =
      !focused || isSongsTab || isBibleTab ? "" : "none";
  if (presetBtn)
    presetBtn.style.display = !focused || !isScheduleTab ? "" : "none";
}
...
function updateFocusedEditorBanner() {
  ...
  if (sidebarTab === "songs") {
    ...
    subtitleEl.textContent =
      currentItem && !getIsBibleItem(currentItem)
    ...
    if (currentItem && !getIsBibleItem(currentItem)) {
      ...
      const getSectionLabel = (tag) => {
        ...
        const match = trimmed.match(
          /^(verse|chorus|bridge|refrain|pre[-\s]?chorus|intro|outro|tag|hook)\b/i,
        );
      ...
      renderSectionNav(
        sectionItems.filter(
          (item) => item.label && item.label.toLowerCase() !== "lyrics",
        ),
      );
    } else {
      renderSectionNav([]);
...
function jumpToFocusedSongSection(tag) {
  if (sidebarTab !== "songs" || !currentItem || getIsBibleItem(currentItem))
    return;
...
function setupFocusedMainPanelEvents() {
  ...
  panel.addEventListener("click", (event) => {
    ...
    if (actionBtn) {
      event.stopPropagation();
      handleFocusedSchedulePanelAction(
        actionBtn.dataset.focusedAction,
        actionBtn.dataset.focusedScheduleIndex,
      );
      return;
    }
    ...
    if (row) {
      handleFocusedSchedulePanelAction(
        "select",
        row.dataset.focusedScheduleIndex,
      );
    }
  });
...
function shouldUseDockedSidebarLayout() {
  ...
  const sidebarWidth = Math.max(
    ...
    Number.isFinite(cssWidth) ? cssWidth : 200,
  );
...
function applySidebarLayoutMode(layout, opts = {}) {
  ...
  if (select && select.value !== sidebarLayoutMode)
    select.value = sidebarLayoutMode;
...
function positionSidebarPopup() {
  ...
  const left = activityRect ? activityRect.right : hostRect ? hostRect.left : 0;
...
function setSidebarPopupOpen(open) {
  ...
  if (isSidebarDockedLayoutActive()) {
    ...
    try {
      localStorage.setItem("sidebarOpen", shouldOpen ? "1" : "0");
    } catch (e) {}
    updateSidebarToggleTooltip();
    positionSidebarQuickActions();
    return;
